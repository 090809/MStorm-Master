...
bool Battlefield::Update(uint32 diff)
{
    if (m_Timer <= diff)
    {
		...
		if (IsWarTime()) {
			EndBattle(true);
			if (m_BattleId == BATTLEFIELD_BATTLEID_WG) {
				sWorld->SendWorldText(6003);
				if (GameObjectData const* gameObjectData = sObjectMgr->GetGOData(192039)) {
					GameObject* obj = sMapMgr->FindBaseNonInstanceMap(gameObjectData->mapid)->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, gameObjectData->id, uint32(192039)));
					if (obj && !obj->HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE))
    ...
    if (!IsWarTime() && !m_StartGrouping && m_Timer <= m_StartGroupingTimer)
    {
        ...
        OnStartGrouping();
		if (m_BattleId == BATTLEFIELD_BATTLEID_WG) {
			sWorld->SendWorldText(6000);
			if (GameObjectData const* gameObjectData = sObjectMgr->GetGOData(192039)) {
				GameObject* obj = sMapMgr->FindBaseNonInstanceMap(gameObjectData->mapid)->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, gameObjectData->id, uint32(192039))); 
				if (obj && obj->HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE))
					obj->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_IN_USE);
			}
		}
    }
