...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS]=
{
    ...
    &Spell::EffectApplyAreaAura,                                     //163 unused
...
void Spell::EffectSchoolDMG(SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1]&0x00040000)
                {
                    ...
                    if (Player* player = m_caster->ToPlayer()) // UNIT_FIELD_MINDAMAGE/MAXDAMAGE already include damage bonuses, so try to get them without damage bonuses
                        player->CalculateMinMaxDamage(BASE_ATTACK, false, true, min_damage, max_damage);

                    float average = (min_damage + max_damage) / 2;
                    // Add main hand dps * effect[2] amount
                    int64 count = m_caster->CalculateSpellDamage(unitTarget, m_spellInfo, EFFECT_2);
					if (count < 1) count = 1;
                    damage += count * int64(average * IN_MILLISECONDS) / m_caster->GetAttackTime(BASE_ATTACK);
                    break;
                }
			...
			{
				if (m_spellInfo->SpellIconID == 6079)
				{
					// Клинок Исполина наносит на 40% больше урона, если противник находится под действием 
					if (unitTarget->HasAura(300006)) //Смешанные чувства
					{
						damage += int32(damage * 0.4f);
					}
				}
				break;
			}
...
void Spell::EffectLearnSkill(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->SetSkill(skillid, m_spellInfo->Effects[effIndex].CalcValue(), tier->MaxSkill[damage - 1], tier->MaxSkill[damage - 1]);
}
