...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
	...
	Unit::AuraEffectList const& FreeForMoveSpellList = m_caster->GetAuraEffectsByType(SPELL_AURA_FREE_FOR_MOVE);
	for (Unit::AuraEffectList::const_iterator i = FreeForMoveSpellList.begin(); i != FreeForMoveSpellList.end(); ++i)
	{
		//m_caster->Talk("FreeForMove dropped on 1", CHAT_MSG_SAY, LANG_UNIVERSAL, 10, NULL);
		if ((*i)->GetMiscValue() == m_spellInfo->Id)
		{
			FreeForMove = true;
			break;
		}
...
void Spell::update(uint32 difftime)
{
	...
	for (Unit::AuraEffectList::const_iterator i = FreeForMoveSpellList.begin(); i != FreeForMoveSpellList.end(); ++i)
	{
		//m_caster->Talk("FreeForMove dropped on 2", CHAT_MSG_SAY, LANG_UNIVERSAL, 10, NULL);
		if ((*i)->GetMiscValue() == m_spellInfo->Id)
		{
			FreeForMove = true;
			break;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (!m_spellInfo->IsPassive())
    {
        ...
        if (!m_caster->GetSpellHistory()->IsReady(m_spellInfo))
        {
            if (m_triggeredByAuraSpell)
				...
				if (m_spellInfo->Id == 15473
				 || m_spellInfo->Id == 46584) 
				{
					// Send activate cooldown timer (possible 0) at client side
					WorldPacket data(SMSG_COOLDOWN_EVENT, 4 + 8);
					data << uint32(m_spellInfo->Id);
					data << uint64(m_caster->GetGUID());
					GetCaster()->ToPlayer()->SendDirectMessage(&data);
					return SPELL_FAILED_FIZZLE; 
				}
				else { return SPELL_FAILED_NOT_READY; }
        }
    }
	...
	for (Unit::AuraEffectList::const_iterator i = FreeForMoveSpellList.begin(); i != FreeForMoveSpellList.end(); ++i)
	{
		//m_caster->Talk("FreeForMove dropped on 3", CHAT_MSG_SAY, LANG_UNIVERSAL, 10, NULL);
		if ((*i)->GetMiscValue() == m_spellInfo->Id)
		{
			FreeForMove = true;
			break;
