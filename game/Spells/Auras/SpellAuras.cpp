...
void Aura::HandleAuraSpecificMods(AuraApplication const* aurApp, Unit* caster, bool apply, bool onReapply)
{
		...
		case SPELLFAMILY_PRIEST:
			...
			if (GetSpellInfo()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0))
			{
				...
				if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 3790, 1))
				{
					...
					if (!target->HasAuraType(SPELL_AURA_REFLECT_SPELLS))
						caster->CastCustomSpell(target, 63675, &basepoints0, NULL, NULL, true, NULL, GetEffect(0));
					else
						target->CastCustomSpell(caster, 63675, &basepoints0, NULL, NULL, true, NULL, GetEffect(0));
		...
		case SPELLFAMILY_MAGE:
			...
			case 66: // Invisibility
				if (removeMode != AURA_REMOVE_BY_EXPIRE)
					break;
				if (target->HasSpell(200240)) target->AddAura(200241, target);
				if (target->HasSpell(200242)) target->AddAura(200243, target);
				if (target->HasSpell(200244)) target->AddAura(200245, target);
				target->CastSpell(target, 32612, true, NULL, GetEffect(1));
				target->CombatStop();
				break;
			case 32612:
				target->RemoveAurasDueToSpell(200241);
				target->RemoveAurasDueToSpell(200243);
				target->RemoveAurasDueToSpell(200245);
				break;
	...
	case SPELLFAMILY_ROGUE:
		...
		if (GetSpellInfo()->SpellFamilyFlags[0] & 0x00400000)
		{
			...
			else if (!apply && target->HasAura(58427)) target->RemoveAura(58427);
			break;
		}
		break;
	case SPELLFAMILY_HUNTER:
	...
	for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
	{
		...
		else
		{
			...
			if (!GetUnitOwner()->HasUnitState(UNIT_STATE_ISOLATED))
			{
				...
				case SPELL_EFFECT_163://SPELL_EFFECT_APPLY_AREA_AURA_NOT_HOSTILE
				{
					Trinity::AnyNonHostileUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius); // No GetCharmer in searcher
					Trinity::UnitListSearcher<Trinity::AnyNonHostileUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
					GetUnitOwner()->VisitNearbyObject(radius, searcher);
					break;
				}
