...
enum GuildBonus
{
	...
	GUILD_BONUS_ARENA_POINTS_1,
	GUILD_BONUS_ARENA_POINTS_2,
	GUILD_BONUS_ARENA_POINTS_3,

	//Chances - 10%
	//		  - 20%
	//		  - 30%

	GUILD_BONUS_GUILD_GOLD_1,
	GUILD_BONUS_GUILD_GOLD_2,
	GUILD_BONUS_GUILD_GOLD_3,
...
#define GUILD_MAX_LEVEL sWorld->GetMaxGuildLevel()
...
class GuildTalentsSystem
{
public:

	struct TreeLevel
	{
		uint8 required_guild_level, tree_level;
		uint32 spells[3];
		TreeLevel(uint32 _tree_level, uint32 _spell_1, uint32 _spell_2, uint32 _spell_3, uint32 _guild_level)
		{
			tree_level = _tree_level;
			spells[0] = _spell_1;
			spells[1] = _spell_2;
			spells[2] = _spell_3;
			required_guild_level = _guild_level;
		}
	};
	struct TreeLevelSorter
	{
		bool operator() (TreeLevel first, TreeLevel second)
		{
			return first.tree_level < second.tree_level;
		}
	} TreeLevelSorter;

	struct TreeLevelV
	{
		uint8 _tree_level, _talent_id;
		TreeLevelV(uint32 tree_level, uint32 talent_id)
		{
	...
	struct TreeLevelVSorter
	{
		bool operator() (TreeLevelV first, TreeLevelV second)
		{
			return first._tree_level < second._tree_level;
		}
	} TreeLevelVSorter;

	void Load()
	{
		QueryResult result = WorldDatabase.PQuery("SELECT tree_level, spell_1, spell_2, spell_3, required_guild_level FROM guild_tree_system ORDER BY tree_level ASC");
		if (result)
			do
			{
				Field* fields = result->Fetch();
				uint32 tree_level = fields[0].GetUInt32();
				uint32 spell_1 = fields[1].GetUInt32();
				uint32 spell_2 = fields[2].GetUInt32();
				uint32 spell_3 = fields[3].GetUInt32();
				uint32 guild_level = fields[4].GetUInt32();
				TreeLevelVector.push_back(TreeLevel(tree_level, spell_1, spell_2, spell_3, guild_level));
			} while (result->NextRow());

			//Избегаем ошибки компиляции и исполнения! Если внутри, по какой-то причине, не оказалось талантов - выходим из цикла.
		if (TreeLevelVector.size() > 1)
			std::sort(TreeLevelVector.begin(), TreeLevelVector.end(), TreeLevelSorter);
	}
	std::vector < TreeLevel > GetAllGuildTreeLevels() { return TreeLevelVector; }

private:
	std::vector<TreeLevel> TreeLevelVector;
};
...
public:
	...
	uint8 GetGuildLevel() const { return m_guild_level; };
	bool IsRoyalGuild();
	std::vector<GuildTalentsSystem::TreeLevelV> GetGuildTalents() { return m_TreeLevelVector; }
	uint16 GetMaxGuildTreeLevel() { return MaxGuildTreeLevel; }
	void UpdateSpellsOnlinePlayers();
	uint16 GetTalentID(uint32 TreeLevel);
	void SetGuildTalent(uint32 TreeLevel, uint32 TalentId);
protected:
    uint32 m_id;
    std::string m_name;
	...
	std::vector<GuildTalentsSystem::TreeLevelV> m_TreeLevelVector;
	uint8 MaxGuildTreeLevel;
private:
