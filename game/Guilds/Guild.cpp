...
void Guild::LoadTalentsInfo()
{
	std::vector<GuildTalentsSystem::TreeLevel> maxtalents = sWorld->GetGuildTalentsSystem()->GetAllGuildTreeLevels();
	QueryResult result = CharacterDatabase.PQuery("SELECT tree_level, talent_id FROM guild_talents WHERE guild_id = %u ORDER BY tree_level ASC", GetId());
	if (result)
	{
		do
		{
...
bool Guild::IsRoyalGuild() {
	PreparedStatement* stmt;
	stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ROYAL_GUILD);
	PreparedQueryResult result = CharacterDatabase.Query(stmt);

	return (*result)[0].GetUInt32() == m_id;
}
...
void Guild::SetLevel(uint8 level, bool byCommand)
{
    ...
    char worldMsg[250];
    sprintf(worldMsg, "Гильдия %s достигла %d уровня.", m_name.c_str(), level);
