...
void World::LoadConfigSettings(bool reload)
{
	...
	m_bool_configs[SHOW_CINEMATIC_ON_FIRST_LOGIN] = sConfigMgr->GetBoolDefault("ShowCinematicsOnFirstLogon", false);
    ...
    if (m_int_configs[CONFIG_MAX_PLAYER_LEVEL] > (MAX_LEVEL+20))
    {
        ...
        m_int_configs[CONFIG_MAX_PLAYER_LEVEL] = (MAX_LEVEL+20);
    }
	...
	//reward_id
	m_int_configs[COMMANDS_REWARD_ID] = sConfigMgr->GetIntDefault("Reward.Id", 34597);
	//GoldPlagueEventGold
	m_int_configs[CONFIG_GOLD_PLAG_GOLD_COUNT] = sConfigMgr->GetIntDefault("Events.GoldPlague.GoldCount", 10);

	//BG reward
	m_int_configs[CONFIG_BG_REWARD_ID] = sConfigMgr->GetIntDefault("BGReward.ID", 0);
	m_int_configs[CONFIG_BG_REWARD_WIN_COUNT] = sConfigMgr->GetIntDefault("BGReward.WinCount", 0);
	m_int_configs[CONFIG_BG_REWARD_LOSE_COUNT] = sConfigMgr->GetIntDefault("BGReward.LoseCount", 0);
    ...
    m_int_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_MAX_TIMER] = (sConfigMgr->GetIntDefault("ModifierOnlineCharacters.MaxTimer", 60) * MINUTE * IN_MILLISECONDS);
	m_int_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_ITEM_ID] = sConfigMgr->GetIntDefault("ModifierOnlineCharacters.ItemID", 41596);
	m_int_configs[CONFIG_MODIFIER_ONLINE_CHARACTERS_ITEM_COUNT] = sConfigMgr->GetIntDefault("ModifierOnlineCharacters.ItemCount", 1);

	// Last bost rewards
	m_int_configs[CONFIG_LAST_BOSS_REWARD_POINT_MINIMUM_PLAYERS] = sConfigMgr->GetBoolDefault("LastBossReward.MinimumPlayers", 3);
	m_int_configs[CONFIG_LAST_BOSS_REWARD_POINT_NEWBEE] = sConfigMgr->GetBoolDefault("LastBossReward.NewbeeExcepts", 10);
	m_int_configs[CONFIG_LAST_BOSS_REWARD_POINT_PRO] = sConfigMgr->GetBoolDefault("LastBossReward.ProExcepts", 20);
	m_int_configs[CONFIG_LAST_BOSS_REWARD_POINT_ID] = sConfigMgr->GetBoolDefault("LastBossReward.PointID", 34822);
	m_int_configs[CONFIG_LAST_BOSS_REWARD_POINT_BIGGEST] = sConfigMgr->GetBoolDefault("LastBossReward.GoodPartyPointsCount", 10);
	m_int_configs[CONFIG_LAST_BOSS_REWARD_POINT_LOWEST] = sConfigMgr->GetBoolDefault("LastBossReward.BadPartyPointsCount", 5);
...
void World::SetInitialWorldSettings()
{
	...
	QueryResult result = CharacterDatabase.PQuery("SELECT MAX(level) FROM guild_xp_for_next_level");
	_maxGuildLevel = (*result)[0].GetUInt8() + 1;

	_GuildTalentsSystem = new GuildTalentsSystem();
	_GuildTalentsSystem->Load();
	...
	TC_LOG_INFO("server.loading", "Cleaning `promotion_codes_used` table...");
	CharacterDatabase.PQuery("DELETE FROM promotion_codes_used WHERE code IN (SELECT code FROM promotion_codes WHERE code_count <= 0);");
	...
	if (m_timers[WUPDATE_MODIFIER_ONLINE_CHARACTERS].Passed())
    {
        ...
        if (getBoolConfig(CONFIG_MODIFIER_ONLINE_CHARACTERS_ENABLE))
        {
            ...
            for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
            {
				...
				//sWorld->getIntConfig(CONFIG_MODIFIER_ONLINE_CHARACTERS_ITEM_ID);

				uint8 additional_points_count = 0;
				if (player->IsVIP())
					additional_points_count += 2;

				player->AddItem(
					sWorld->getIntConfig(CONFIG_MODIFIER_ONLINE_CHARACTERS_ITEM_ID),
					sWorld->getIntConfig(CONFIG_MODIFIER_ONLINE_CHARACTERS_ITEM_COUNT) + additional_points_count
				);
				//player->AddItem(1, 1);
				ChatHandler(itr->second).PSendSysMessage(LANG_MODIFIER_ONLINE_CHARACTERS_WHISPER);
				//ChatHandler(itr->second).PSendSysMessage(LANG_MODIFIER_ONLINE_CHARACTERS_WHISPER, sTransmogrification->GetItemLink(CONFIG_MODIFIER_ONLINE_CHARACTERS_ITEM_ID, player->GetSession()));
            }
        }
    }
