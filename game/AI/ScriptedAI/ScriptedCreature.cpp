...
{
	Player* player;
	uint16 count_of_instance;
};

//Выдача призов за убийство последнего босса
void BossAI::GiveRewardPoints()
{
	...
	uint16 count_of_newbees = 0;
	uint16 player_count = 0;
	std::vector<RewardMap> RewMap;
	
	uint8 minimum_players_to_reward = sWorld->getIntConfig(CONFIG_LAST_BOSS_REWARD_POINT_MINIMUM_PLAYERS);
	uint16 tryes_while_newbee = sWorld->getIntConfig(CONFIG_LAST_BOSS_REWARD_POINT_NEWBEE);
	uint16 tryes_pro = sWorld->getIntConfig(CONFIG_LAST_BOSS_REWARD_POINT_PRO);
	uint32 reward_item_id = sWorld->getIntConfig(CONFIG_LAST_BOSS_REWARD_POINT_ID);
	uint8 biggest_reward = sWorld->getIntConfig(CONFIG_LAST_BOSS_REWARD_POINT_BIGGEST);
	uint8 lowest_reward = sWorld->getIntConfig(CONFIG_LAST_BOSS_REWARD_POINT_LOWEST);
	...
	for (std::list<Player*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
	{
		...
		if (CountOfInstance)
		{
			...
			Rewarder.count_of_instance = fields[0].GetInt32() + 1;
		}
		else Rewarder.count_of_instance = 1;
		
		if (Rewarder.count_of_instance <= tryes_while_newbee)
		{
			bool dooble = false;
			for (std::list<Player*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
			{
				if (player != (*iter) && player->GetSession()->GetRemoteAddress() == (*iter)->GetSession()->GetRemoteAddress())
				{
					dooble = true;
					break;
				}
		...
		CharacterDatabase.PExecute("INSERT INTO instance_reward_data (instance_id, player_id, account_id, count_of_instance) VALUES (%u, %u, %u, %u);", me->GetInstanceScript()->instance->GetId(), player->GetGUIDLow(), player->GetSession()->GetAccountId(), Rewarder.count_of_instance);
		RewMap.push_back(Rewarder);
		player_count += 1;
	}

	if (RewMap.size() >= minimum_players_to_reward)
		for (uint8 i = 0; i < RewMap.size(); i++)
		{
			if (RewMap[i].count_of_instance >= tryes_pro)
			{
				if (player_count - count_of_newbees <= count_of_newbees)
				{
					RewMap[i].player->AddItem(reward_item_id, biggest_reward);
				}
				else RewMap[i].player->AddItem(reward_item_id, lowest_reward);
			}
		}
}
