...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    recvData >> level_min;                                 // minimal player level, default 0
    recvData >> level_max;                                 // maximal player level, default 100 (MAX_LEVEL)
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        if (!(classmask & (1 << class_)) && class_ != CLASS_MYSTIC)
            continue;
