...
void WorldSession::HandleLootMoneyOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (loot)
    {
        ...
        if (shareMoney && player->GetGroup())      //item, pickpocket and players can be looted only single player
        {
            ...
            {
				uint32 mod_gold = 0 + uint32((*i)->GetMaxPositiveAuraModifier(SPELL_AURA_MOD_ADDITIONAL_MONEY_PCT) * goldPerPlayer / 100);
                (*i)->ModifyMoney(goldPerPlayer + mod_gold);
		...
		{
			uint32 mod_gold = 0 + uint32(player->GetMaxPositiveAuraModifier(SPELL_AURA_MOD_ADDITIONAL_MONEY_PCT) * loot->gold / 100);

			player->ModifyMoney(loot->gold + mod_gold);
		...
		/*if (Guild* guild = player->GetGuild())
        {
			...
			if (guild->HasLevelForBonus(GUILD_BONUS_GUILD_GOLD_1))
                guild->HandleMemberDepositMoney(this, uint32(loot->gold*0.1f));
			if (guild->HasLevelForBonus(GUILD_BONUS_GUILD_GOLD_2))
				guild->HandleMemberDepositMoney(this, uint32(loot->gold*0.2f));
			if (guild->HasLevelForBonus(GUILD_BONUS_GUILD_GOLD_3))
				guild->HandleMemberDepositMoney(this, uint32(loot->gold*0.3f));
        }
		*/
