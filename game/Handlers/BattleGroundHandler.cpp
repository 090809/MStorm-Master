...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    if (action)
    {
		...
		if (bg->isArena()) {
			...
			for (std::map<ObjectGuid, PlayerQueueInfo>::iterator itr = bgQueue.m_QueuedPlayers.begin(); itr != bgQueue.m_QueuedPlayers.end(); ++itr)
			{
				// get the player
				Player* player_in_queue = ObjectAccessor::FindConnectedPlayer(itr->first);
				if (player_in_queue && _player && player_in_queue != _player && player_in_queue->GetSession()->GetRemoteAddress() == _player->GetSession()->GetRemoteAddress())
				{
					ChatHandler(player_in_queue->GetSession()).SendSysMessage("Someone with your IP trying to join queue");
					ChatHandler(_player->GetSession()).SendSysMessage("Someone with your IP is already in queue");
					return;
				}
			};
			...
			for (std::map<ObjectGuid, BattlegroundPlayer>::const_iterator itr = bg->GetPlayers().begin(); itr != bg->GetPlayers().end(); ++itr)
			{
				...
				if (player && _player && player != _player && player->GetSession()->GetRemoteAddress() == _player->GetSession()->GetRemoteAddress())
				{
					ChatHandler(player->GetSession()).SendSysMessage("Someone with your IP trying to join queue");
					ChatHandler(_player->GetSession()).SendSysMessage("Someone with your IP is already in queue");
					return;
				}
			};
		...
		//ChangeEquipToHighLevelIfNeed(_player);
...
void WorldSession::HandleBattlefieldLeaveOpcode(WorldPacket& recvData)
{
	...
	if (_player->m_player_changedbgitems)
		_player->ChangeBackItems();
