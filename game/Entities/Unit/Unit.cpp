...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
	...
	if (GetTypeId() == TYPEID_PLAYER || (GetTypeId() == TYPEID_UNIT && IsHunterPet()))
    {
        float bonusPct = 0;
		...
		if (GetTypeId() == TYPEID_UNIT && IsHunterPet()) {
			armorPen = CalculatePct(maxArmorPen, bonusPct + GetOwner()->ToPlayer()->GetRatingBonusValue(CR_ARMOR_PENETRATION));
		} else if (GetTypeId() == TYPEID_PLAYER) {
			armorPen = CalculatePct(maxArmorPen, bonusPct + ToPlayer()->GetRatingBonusValue(CR_ARMOR_PENETRATION));
		}
...
float Unit::CalculateLevelPenalty(SpellInfo const* spellProto) const
{
	...
	float level = float(std::min(int(getLevel()), 80));
    float LvlFactor = (float(spellProto->SpellLevel) + 6.0f / level);
    if (LvlFactor > 1.0f)
        LvlFactor = 1.0f;
...
void Unit::_UpdateSpells(uint32 time)
{
	...
	///@TODO Переделать данную штуку под работу "Визуальность только для ГМ"?
	// На самом деле - достаточно непонятно, можно ли это адекватно переписать для Визуальности только ГМ+игрок. Стандарт клиент модель это не позволяет
	// Попробовать вынести в отдельные массивы?
...
{
	bool FreeForMove = false;

	Unit::AuraEffectList const& FreeForMoveSpellList = GetAuraEffectsByType(SPELL_AURA_FREE_FOR_MOVE);
	for (Unit::AuraEffectList::const_iterator i = FreeForMoveSpellList.begin(); i != FreeForMoveSpellList.end(); ++i)
	{
		//Talk("FreeForMove dropped on 4", CHAT_MSG_SAY, LANG_UNIVERSAL, 10, NULL);
		if ((*i)->GetMiscValue() == m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id)
		{
			FreeForMove = true;
			break;
		}
	...
	if ((GetTypeId() == TYPEID_PLAYER && ToPlayer()->isMoving() && !FreeForMove) || IsNonMeleeSpellCast(false, false, true, m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id == 75))
    {
        // cancel wand shoot
...
void Unit::RemoveNotOwnSingleTargetAuras(uint32 newPhase)
{
    ...
    {
        Aura* aura = *iter;
		if (aura->GetUnitOwner() != this && !aura->GetUnitOwner()->InSamePhase(newPhase)) //Для паладинов, что бы не убиралось правосудие
        {
            aura->Remove();
            iter = scAuras.begin();
        }
...
void Unit::RemoveAllAurasExceptType(AuraType type1, AuraType type2, uint32 spellid_1, uint32 spellid_2)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        Aura const* aura = iter->second->GetBase();
        if (aura->GetSpellInfo()->HasAura(type1) || aura->GetSpellInfo()->HasAura(type2) || aura->GetSpellInfo()->Id == spellid_1 || aura->GetSpellInfo()->Id == spellid_2)
            ++iter;
        else
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
    }
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            switch (dummySpell->Id)
            {
				...
				case 200021: // Карающая длань 1
				case 200022: // Карающая длань 2
				case 200023: // Карающая длань 3
				{
					if (!victim || !victim->IsAlive())
						return false;

					CastSpell(this, 200024, true, NULL, triggeredByAura);

					Aura const* dummy = GetAura(200024);
					if (!dummy || dummy->GetStackAmount() < 6)
						return false;

					RemoveAurasDueToSpell(200024);
					triggered_spell_id = 200025;  // default main hand attack
					target = victim;
					break;
				}
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->Id == 49028)
			{
				...
				Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(pPet, pPet, 30.0f);
				Trinity::UnitListSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> searcher(pPet, targets, u_check);
				pPet->VisitNearbyObject(30.0f, searcher);
				for (std::list<Unit*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
					if ((*iter)->HasAura(49028, pPet->GetOwnerGUID()))
					{
						Victim = (*iter);
						pPet->Attack(Victim, true);
						pPet->CombatStart(Victim, true);
						pPet->AddThreat(Victim, 1000.0f);
						pPet->SetInCombatWith(Victim);
						VictimSelected = true;
						break;
					}
				if (!VictimSelected && pPet) {
					Victim = victim;
					pPet->Attack(Victim, true);
					pPet->CombatStart(Victim, true);
					pPet->AddThreat(Victim, 1000.0f);
				...
				pPet->GetMotionMaster()->MoveFollow(Victim, 3.0f, 0);
				if (pPet && Victim && Victim->IsAlive())
				{
					if (procSpell)
					{
						if (procSpell->Id == 48982 || procSpell->Id == 47568 || procSpell->Id == 49016 || procSpell->Id == 59754)
							return false;
						if (procSpell->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && (procSpell->SpellFamilyFlags[EFFECT_0] & 0x1))        // DRW cast disease if spell is Plague Strike or Icy Touch или Ледяные оковы
							pPet->CastSpell(Victim, 55078, true);
						else if (procSpell->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && ((procSpell->SpellFamilyFlags[EFFECT_0] & 0x2) || (procSpell->Id == 45524)))
							pPet->CastSpell(Victim, 55095, true);
						
						if (!damage) 
							return false;
						
						uint32 procDmg = uint32(damage * 0.5f); //Руник сворд копирует 50% урона дк.
						pPet->SendSpellNonMeleeDamageLog(Victim, procSpell->Id, procDmg, procSpell->GetSchoolMask(), 0, 0, false, 0, false);
						pPet->DealDamage(Victim, procDmg, NULL, SPELL_DIRECT_DAMAGE, procSpell->GetSchoolMask(), procSpell, true);
					...
					else    //Руник сворд копирует 100% урона дк.
					{
						if (!damage)
							return false;
						...
						damageInfo.damage = uint32(damageInfo.damage * 0.5f);
						// Send log damage message to client
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 7293:
            {
				...
				if (spellProto->SpellFamilyFlags.HasFlag(0x2000))
					AddPct(DoneTotalMod, (*i)->GetSpellInfo()->GetRank() * 8.0f);
                break;
            }
            // Twisted Faith
            case 7377:
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    if (TakenTotalCasterMod && !HasAura(700000)) ///@Hack. Игнорирование любого "снижения" урона, если на нас есть занижение урона от 700000 ауры...
    {
        if (TakenTotal < 0)
        {
            if (TakenTotalMod < 1)
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack) const
{
	...
	///@HACK: Снижение самохила рестор-друидов, холи-паладинов, рестор-шаман на 40% по себе.
	if (caster == this
		&& caster->ToPlayer()
		&& caster->getClass() == CLASS_DRUID)
	{
		...
		for (uint32 talentId = 0; talentId < sTalentStore.GetNumRows(); ++talentId)
		{
			...
			for (int8 rank = MAX_TALENT_RANK - 1; rank >= 0; --rank)
			{
				...
				{
					if (282 == talentInfo->TalentTab || //Друид - исцеление
						382 == talentInfo->TalentTab || //Свет - паладины
						202 == talentInfo->TalentTab || //Свет - присты
						262 == talentInfo->TalentTab)   //Исцеление - шаман
						count += rank + 1;
				}
			}
		}
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    if (TakenTotalCasterMod && !HasAura(700000))
    {
...
void Unit::ModSpellCastTime(SpellInfo const* spellInfo, int32 & castTime, Spell* spell)
{
	...
	if (spellInfo->IsChanneled() && !spellInfo->HasAttribute(SPELL_ATTR5_HASTE_AFFECT_DURATION) && spellInfo->Id != 6358)
        return;
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && (victim->getClass() == CLASS_PRIEST || victim->getClass() == CLASS_PALADIN))
    {
        AuraEffectList const& dummyAuras = victim->GetAuraEffectsByType(SPELL_AURA_DUMMY);
        for (AuraEffectList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
        {
			if ((*itr)->GetSpellInfo()->SpellIconID == 1654 || (*itr)->GetSpellInfo()->Id == 200030 || (*itr)->GetSpellInfo()->Id == 200031)
            {
				...
				if ((*itr)->GetSpellInfo()->Id == 200030)
					victim->CastSpell(victim, 200032, true, NULL, aurEff);
				else if ((*itr)->GetSpellInfo()->Id == 200031)
					victim->CastSpell(victim, 200033, true, NULL, aurEff);
				else 
					victim->CastSpell(victim, 27827, true, NULL, aurEff);

				victim->CastSpell(victim, 4309, true, NULL, aurEff); // Накладывает immune к любому урону
...
float Unit::GetCombatRatingReduction(CombatRating cr, float cap) const
{
    if (Player const* player = ToPlayer())
		return std::min(player->GetRatingBonusValue(cr), cap); //7.535977243834549
    // Player's pet get resilience from owner
    else if (IsPet() && GetOwner())
        if (Player* owner = GetOwner()->ToPlayer())
            return std::min(owner->GetRatingBonusValue(cr), cap);
...
{
    float percent = std::min(GetCombatRatingReduction(cr) * rate, cap); //15.0719544876691
	TC_LOG_DEBUG("entities.unit", "GetCombatRatingDamageReduction: CombatRating ID: %u, rate: %f, cap: %f, damage: %u, percent: %f", cr, rate, cap, damage, percent);
