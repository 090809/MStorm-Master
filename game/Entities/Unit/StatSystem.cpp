...
void Player::UpdateArmor()
{
    ...
    value += GetStat(STAT_AGILITY) * 0;						// armor bonus from stats -- Нет бонуса для брони от Агилы
...
void Player::UpdateCritPercentage(WeaponAttackType attType)
{
	...
	float crcr = GetRatingBonusValue(cr);
	float value = GetTotalPercentageModValue(modGroup) + crcr;
...
/*
    ...
    else if (stat == STAT_STAMINA)
    {
		...
		ownersBonus = CalculatePct(owner->GetStat(STAT_AGILITY), 75);
		value += ownersBonus;
	}
                                                            //warlock's and mage's pets gain 30% of owner's intellect
    else if (stat == STAT_INTELLECT)
    {
		if (owner->getClass() == CLASS_WARLOCK || owner->getClass() == CLASS_MAGE || owner->getClass() == CLASS_HUNTER || owner->getClass() == CLASS_MYSTIC)
        {
            ownersBonus = CalculatePct(owner->GetStat(stat), 30);
            value += ownersBonus;
        }
...
void Guardian::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (owner && owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            bonusAP = owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.451f;
            SetBonusDamage(int32(owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.1f));
        }
        else if (IsSpiritWolf()) //wolf benefit from shaman's attack power
        {
            float dmg_multiplier = 0.2f;
        ...
        else if (IsPet())
        {
            ...
            if (maximum < 0)
                maximum = 0;
            SetBonusDamage(int32(maximum * 0.4f));
            bonusAP = maximum * 0.96317f;
        }
        ...
        else if (GetEntry() == ENTRY_WATER_ELEMENTAL)
        {
            ...
            if (frost < 0)
                frost = 0;
            SetBonusDamage(int32(frost * 0.63f));
        }
    }


    /*uint32 entry = IsHunterPet() ? 1 : GetEntry();
    PetLevelInfo const* pInfo = sOsbjectMgr->GetPetLevelInfo(entry, getLevel());
...
float Guardian::GetMeleeCritFromAgility()
{
    ...
    float crit = critBase->base + 0*GetStat(STAT_AGILITY)*critRatio->ratio;
    return crit*100.0f;
}
