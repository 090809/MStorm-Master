...
Player::Player(WorldSession* session): Unit(true)
{
	...
	std::vector<BGItems> SavedBGItems = {};
}
...
bool Player::Create(uint32 guidlow, CharacterCreateInfo* createInfo)
{
	...
	PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_VIP);
...
void Player::Update(uint32 p_time)
{
	...
	if (IsVIP() && now > m_VIP_timeto && m_VIP_timeto != -1)
		RemoveVIP();
...
void Player::LearnSpell(uint32 spell_id, bool dependent, bool fromSkill /*= false*/)
{
	...
	sScriptMgr->OnPlayerSpellLearn(this, spell_id);
...
void Player::RemoveSpell(uint32 spell_id, bool disabled, bool learn_low_rank)
{
    ...
    if ((spell_id == 674 || spell_id == 300100)  && m_canDualWield)
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
	...
	if (getClass() == CLASS_MYSTIC) {
		if (HasAura(300012)) RemoveAura(300012);
		if (!HasAura(300011)) AddAura(300011, this);
	}
...
{
    float value = (m_auraBaseMod[SHIELD_BLOCK_VALUE][FLAT_MOD] + GetStat(STAT_STRENGTH) * 0.2f - 10)*m_auraBaseMod[SHIELD_BLOCK_VALUE][PCT_MOD];
...
float Player::GetMeleeCritFromAgility()
{
    ...
    GtChanceToMeleeCritEntry     const* critRatio = sGtChanceToMeleeCritStore.LookupEntry((pclass)*GT_MAX_LEVEL + level-1);
    if (critBase == NULL || critRatio == NULL)
        return 0.0f;

    float crit = critBase->base /*+ GetStat(STAT_AGILITY)*critRatio->ratio*/; // Крит от агилы = 0;
    return crit*100.0f;
}
...
void Player::GetDodgeFromAgility(float &diminishing, float &nondiminishing)
{
    ...
    GtChanceToMeleeCritEntry  const* dodgeRatio = sGtChanceToMeleeCritStore.LookupEntry((pclass)*GT_MAX_LEVEL + level-1);
    if (dodgeRatio == NULL || pclass > MAX_CLASSES)
        return;

    /// @todo research if talents/effects that increase total agility by x% should increase non-diminishing part
    float base_agility = GetCreateStat(STAT_AGILITY) * m_auraModifiersGroup[UNIT_MOD_STAT_START + STAT_AGILITY][BASE_PCT];
    float bonus_agility = (GetStat(STAT_AGILITY) - base_agility)*0.0005f;

    // calculate diminishing (green in char screen) and non-diminishing (white) contribution
    diminishing = 100.0f * bonus_agility * dodgeRatio->ratio * crit_to_dodge[pclass];
    nondiminishing = 100.0f * (dodge_base[pclass] + base_agility * dodgeRatio->ratio * crit_to_dodge[pclass]);
}
...
float Player::GetSpellCritFromIntellect()
{
    ...
    GtChanceToSpellCritEntry     const* critRatio = sGtChanceToSpellCritStore.LookupEntry((pclass)*GT_MAX_LEVEL + level-1);
    if (critBase == NULL || critRatio == NULL)
        return 0.0f;

    float crit=critBase->base  /*+ GetStat(STAT_INTELLECT)*critRatio->ratio*/;
	//crit = 0; //Крит от интеллекта = 0
    return crit*100.0f;
}
...
float Player::GetRatingMultiplier(CombatRating cr) const
{
    ...
    return classRating->ratio / Rating->ratio; // 1 / 131.767913818 = 0.007589100950488
}

float Player::GetRatingBonusValue(CombatRating cr) const
{
	TC_LOG_DEBUG("entities.unit", "GetCombatRatingDamageReduction: CombatRating: %u, CombatRatingValue: %f, RatingMultiplier: %f", cr, float(GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + cr)), GetRatingMultiplier(cr));
    return float(GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + cr)) * GetRatingMultiplier(cr); //7.535977243834549 || 0.0212418300653595
}
...
float Player::OCTRegenHPPerSpirit()
{
    ...
    GtOCTRegenHPEntry     const* baseRatio = sGtOCTRegenHPStore.LookupEntry((pclass)*GT_MAX_LEVEL + level-1);
    GtRegenHPPerSptEntry  const* moreRatio = sGtRegenHPPerSptStore.LookupEntry((pclass)*GT_MAX_LEVEL + level-1);
...
//    GtOCTRegenMPEntry     const* baseRatio = sGtOCTRegenMPStore.LookupEntry((pclass-1)*GT_MAX_LEVEL + level-1);
    GtRegenMPPerSptEntry  const* moreRatio = sGtRegenMPPerSptStore.LookupEntry((pclass)*GT_MAX_LEVEL + level-1);
    if (moreRatio == NULL)
        return 0.0f;
...
void Player::ModifyArenaPoints(int32 value, SQLTransaction trans)
{
	/*if (Guild* guild = GetGuild())
	{
		if (guild->HasLevelForBonus(GUILD_BONUS_HONOR_1)) {
			value += uint32(value*0.15);
		}
		else if (guild->HasLevelForBonus(GUILD_BONUS_HONOR_2)) {
			value += uint32(value*0.1);
		}
		else if (guild->HasLevelForBonus(GUILD_BONUS_HONOR_3)) {
			value += uint32(value*0.05);
		}
	}*/
...
InventoryResult Player::CanRollForItemInLFG(ItemTemplate const* proto, WorldObject const* lootedObject) const
{
   ...
   // Used by group, function NeedBeforeGreed, to know if a prototype can be used by a player
    ...
    const static uint32 item_weapon_skills[MAX_ITEM_SUBCLASS_WEAPON] =
    {
        ...
        SKILL_2H_MACES, SKILL_POLEARMS, SKILL_SWORDS,        SKILL_2H_SWORDS, SKILL_MAGIC_SWORDS,
    ...
    if (proto->Class == ITEM_CLASS_ARMOR && proto->SubClass > ITEM_SUBCLASS_ARMOR_MISC && proto->SubClass < ITEM_SUBCLASS_ARMOR_BUCKLER && proto->InventoryType != INVTYPE_CLOAK)
    {
        ...
        if (_class == CLASS_MAGE || _class == CLASS_PRIEST || _class == CLASS_WARLOCK || _class == CLASS_MYSTIC)
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    if (moneyRew)
    {
		uint32 mod_gold = 0 + uint32(GetMaxPositiveAuraModifier(SPELL_AURA_MOD_ADDITIONAL_MONEY_PCT) * moneyRew / 100);
        ModifyMoney(moneyRew + mod_gold);
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
	...
	PreparedStatement* stmt_q = LoginDatabase.GetPreparedStatement(LOGIN_SEL_VIP);
	stmt_q->setUInt32(0, GetSession()->GetAccountId());
	stmt_q->setUInt32(0, GetGUIDLow());
	PreparedQueryResult q = LoginDatabase.Query(stmt_q);
	if (q)
	{
		Field* fields = q->Fetch();
		SetVIP(fields[0].GetInt32());
	}
...
uint32 Player::GetBarberShopCost(uint8 newhairstyle, uint8 newhaircolor, uint8 newfacialhair, BarberShopStyleEntry const* newSkin)
{
	...
	if (level > GT_MAX_LEVEL - 20)
		level = GT_MAX_LEVEL - 20;                               // max level in this dbc
...
void Player::StoreLootItem(uint8 lootSlot, Loot* loot)
{
    ...
    if (msg == EQUIP_ERR_OK)
    {
		...
		//GuildLevelSystem Bonuses. @TODO - rewrite to spells
		//start
		uint32 ICount = 0;
		if (item->freeforall) {
			ICount = uint32(item->count * GetMaxPositiveAuraModifier(SPELL_AURA_MOD_FREE_FOR_ALL_LOOT_PCT) / 100);
		}
		//GuildLevelSystem end

		SendNewItem(newitem, uint32(item->count), false, false, true);
		
		//GuildLevelSystem Bonuses start
		if (ICount >= 1)
			AddItem(item->itemid, ICount);
		//GuildLevelSystem end
...
void Player::ActivateSpec(uint8 spec)
{
    ...
    SetPower(pw, 0);
	UpdateArmor();
	if (getClass() == CLASS_DRUID && ( HasSpell(63410) || HasSpell(63411) ))
	{
		RemoveAurasDueToSpell(66530);
		if (GetShapeshiftForm() == FORM_TRAVEL || GetShapeshiftForm() == FORM_NONE) // "while in Travel Form or while not shapeshifted"
			CastSpell(this, 66530, true);
	}
