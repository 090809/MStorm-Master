        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case 19668: // Shadowfiend
                {
                    ...
                    int32 bonus_dmg = int32(GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW)* 0.25f);
                ...
                {
                    SetBonusDamage(int32(GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FROST) * 0.22f));
                ...
                case 27829: // Ebon Gargoyle
                {
                    ...
                    float ownerHaste = p_owner ? p_owner->GetRatingBonusValue(CR_HASTE_MELEE) / 2 : 0;
                    ...
                    int meleeHaste = p_owner? p_owner->GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_HASTE) / 2 : 0;
                    ApplyCastTimePercentMod(meleeHaste, true);
                    //SetBonusDamage(int32(GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * 0.5f));
                    SetBonusDamage(int32(GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * 0.333f));
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - (petlevel / 4)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel + (petlevel / 4)));
                    break;
                }
                ...
                case 89:                // Infernal
                {
                    if (m_owner->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        if (maximum < 0)
                            maximum = 0;
                        SetBonusDamage(int32(maximum * 1.15f));
                    }
                    float mod = 1;
                    if (petlevel < 60)
                        mod = 5 + (petlevel-50)/4;
                ...
                case 11859:                 // Doomguard
				{
                    ...
                    if (mod < 0)
                        mod = 0;
					float minDamage = (int32(maximum * 0.45f) - (petlevel / 4))*mod;
					float maxDamage = (int32(maximum * 0.45f) + (petlevel / 4))*mod;
                    float attackPower = ((minDamage + maxDamage) /4)*7;
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, minDamage);
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, maxDamage);
					SetModifierValue(UNIT_MOD_ATTACK_POWER, BASE_VALUE, attackPower);
					float ownerHaste = 0;
					if (m_owner->ToPlayer())
						ownerHaste = m_owner->ToPlayer()->GetRatingBonusValue(CR_HASTE_MELEE) / 2;
					ApplyPercentModFloatValue(UNIT_MOD_CAST_SPEED, ownerHaste, false);
					ApplyAttackTimePercentMod(BASE_ATTACK, ownerHaste, false);
