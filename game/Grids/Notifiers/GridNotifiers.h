...
namespace Trinity
{
	...
	class AnyNonHostileUnitInObjectRangeCheck
	{
	public:
		AnyNonHostileUnitInObjectRangeCheck(WorldObject const* obj, Unit* unit, float range) : i_obj(obj), i_range(range), i_un(unit) { }
		bool operator()(Unit* u)
		{
			if (u->IsAlive() && !u->IsHostileTo(i_un) && i_obj->IsWithinDistInMap(u, i_range))
				return true;
