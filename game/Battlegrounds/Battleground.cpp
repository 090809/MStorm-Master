...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
		...
		int reward_id = sWorld->getIntConfig(CONFIG_BG_REWARD_ID);

		uint8 additional_points_count = 0;
		if (player->IsVIP())
			additional_points_count += 2;

		if (isBattleground() && reward_id)
			if (team == winner) {
				player->AddItem(reward_id, sWorld->getIntConfig(CONFIG_BG_REWARD_WIN_COUNT) + additional_points_count);
				player->KilledMonsterCredit(599600);
			} else {
				player->AddItem(reward_id, sWorld->getIntConfig(CONFIG_BG_REWARD_LOSE_COUNT) + additional_points_count);
			}
		if (isArena())
			player->KilledMonsterCredit(599601);
...
void Battleground::AddPlayer(Player* player)
{
	...
	player->ChangeEquipToHighLevelIfNeed(isArena());
}
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, float x, float y, float z, float o, TeamId /*teamId = TEAM_NEUTRAL*/, uint32 respawntime /*= 0*/, Transport* transport)
{
	...
	//creature->AddAura(700000, creature); //ƒобавление всем существам ауры 97% урона
