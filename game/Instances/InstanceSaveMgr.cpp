...
#define STANDARTTIME 7200 // 3 HOUR
...
InstanceSave* InstanceSaveManager::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, time_t resetTime, bool canReset, bool load)
{
    ...
    if (!resetTime)
    {
		...
		//else
        {
			resetTime = time(NULL) + STANDARTTIME;
...
void InstanceSaveManager::LoadResetTimes()
{
    ...
    if (result)
    {
        ...
        if (PreparedQueryResult result2 = CharacterDatabase.Query(CharacterDatabase.GetPreparedStatement(CHAR_SEL_MAX_CREATURE_RESPAWNS)))
        {
            do
            {
                ...
                time_t resettime = time_t(fields[0].GetUInt32() + STANDARTTIME);
    ...
    if (result)
    {
        do
        {
            ...
            uint64 newresettime = fields[2].GetUInt32();
			...
			//uint64 newresettime = (oldresettime / STANDARTTIME) * STANDARTTIME;
    ...
    for (MapDifficultyMap::const_iterator itr = sMapDifficultyMap.begin(); itr != sMapDifficultyMap.end(); ++itr)
    {
		...
		//uint32 period = uint32(((mapDiff->resetTime * sWorld->getRate(RATE_INSTANCE_RESET_TIME)) * STANDARTTIME) * STANDARTTIME);
		//if (period < STANDARTTIME)
		//	period = STANDARTTIME;
        ...
        if (!t)
        {
            ...
            //t = today + period + diff;
			t = (now / STANDARTTIME + 1) * STANDARTTIME; //Крези рилм - ресет любого инста - 3 часа от сейчас.
        ...
        if (t < now)
        {
			...
			t = (now / STANDARTTIME + 1) * STANDARTTIME; //Крези рилм - ресет любого инста - 2 часа от сейчас.
			//t = (t / DAY) * DAY;
...
void InstanceSaveManager::_ResetOrWarnAll(uint32 mapid, Difficulty difficulty, bool warn, time_t resetTime)
{
    ...
    if (!warn)
    {
		...
		uint32 next_reset = uint32(((resetTime + MINUTE) / STANDARTTIME * STANDARTTIME) + period + diff);
        //uint32 next_reset = uint32(((resetTime + MINUTE) / DAY * DAY) + period + diff);

        SetResetTimeFor(mapid, difficulty, next_reset);
        ScheduleReset(true, time_t(next_reset), InstResetEvent(1, mapid, difficulty, 0));
