...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_INVENTORY, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, bag, slot, "
                     "item, itemEntry, saved_itemEntry FROM character_inventory ci JOIN item_instance ii ON ci.item = ii.guid WHERE ci.guid = ? ORDER BY bag, slot", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_ITEM_INSTANCE, "REPLACE INTO item_instance (itemEntry, owner_guid, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, saved_itemEntry, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE, "UPDATE item_instance SET itemEntry = ?, owner_guid = ?, creatorGuid = ?, giftCreatorGuid = ?, count = ?, duration = ?, charges = ?, flags = ?, enchantments = ?, randomPropertyId = ?, durability = ?, playedTime = ?, text = ?, saved_itemEntry = ? WHERE guid = ?", CONNECTION_ASYNC);
	...
	PrepareStatement(CHAR_SEL_ROYAL_GUILD, "SELECT guildid FROM guild ORDER by level desc, xp desc LIMIT 1", CONNECTION_SYNCH);
	...
	PrepareStatement(CHAR_UPDATE_BGS_ITEMS_TO_BACKUP, "			UPDATE item_instance SET saved_itemEntry = ?, itemEntry = ? WHERE guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_UPDATE_BGS_ITEMS_TO_BACKUP_DELETE, "	UPDATE item_instance SET saved_itemEntry = 0, itemEntry = ? WHERE guid = ?", CONNECTION_ASYNC);

	PrepareStatement(CHAR_SEL_BG_CHANGES, "SELECT Position, ArenaOnly, id_1, id_2, id_3, id_4, id_5, id_6, id_7, id_8, id_9, id_10, ChangeTo FROM battleground_fake_change WHERE Class = 0 OR Class = ?", CONNECTION_SYNCH);

	PrepareStatement(CHAR_SEL_LEARNED_MOUNTS, "SELECT spell_id FROM account_mounts WHERE account_id = ?", CONNECTION_SYNCH);
	PrepareStatement(CHAR_INS_LEARNED_MOUNTS, "REPLACE INTO account_mounts (account_id, spell_id) VALUES (?, ?)", CONNECTION_ASYNC);
}
