...
public:
	...
	bool OnGossipSelect(Player* player, Creature* me, uint32 uiSender, uint32 uiAction)
	{
		...
		case TREE_MENU:
		{
			for (uint8 i = 0; i < GuildTalents.size(); i++)
			{
				...
				uint32 actionID = i + TREE;
				if (guild->GetMaxGuildTreeLevel() < GuildTalents[i].tree_level)
					break;
				uint8 talent_id = guild->GetTalentID(i + 1);
				//Если у гильдии выбран спелл за этот уровень - отобразить спелл
				if (const SpellInfo* spell = sSpellMgr->GetSpellInfo(GuildTalents[i].spells[talent_id])) {
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, GetSpellIcon(spell->Id) + ' ' + spell->SpellName[session->GetSessionDbcLocale()], i, actionID);
				}
				else
			...
			else if (uiAction >= TREE)
			{
				...
				switch (action)
				{
					...
					{	
						...
						for (uint8 i = 0; i < 3; i++)
						{
							uint32 spell_id = GuildTalents[tree_level].spells[i];
	...
	std::string ConvertToTextTitle(uint32 SItemID, WorldSession* session, uint8 counter)
	{
		return "Обменять " + counter + ' ' + ReturnIconAndName(SItemID, session);
	}

	std::string ConvertToTextBody(uint32 SItemID, WorldSession* session, uint32 uiSender, uint8 counter)
	{
		if (counter == 1)
		...
		return "Вы уверены что хотите обменять \n\n" + counter + ' ' + ReturnIconAndName(SItemID, session) + " на " + std::to_string(ChangerVector[uiSender].XP * counter) + " очка(ов) опыта гильдии?";
	}
