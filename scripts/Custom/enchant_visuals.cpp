...
void SetEnchantVisual(Player* player, Item* item, int32 enchant = -1)
{
    if (!player || !item)
        return;
	...
	if (enchant == -1 || enchant > (int32)ItemEnchants_size)
		enchant = urand(0, ItemEnchants_size);
    enchantStore.AddEnchant(player->GetGUIDLow(), item->GetGUIDLow(), ItemEnchants[enchant]);
