...
public:
	...
	void OnLevelChanged(Player* player, uint8 oldLevel)
	{
		...
		for (TrainerSpellMap::const_iterator itr = t_spells->spellList.begin(); itr != t_spells->spellList.end(); ++itr) {
			...
			if (!spell) 
				continue;
			if ((spell->reqLevel > newLevel) || (player->HasSpell(f_spell)) || spell->reqLevel > 80) 
				continue;
			const SpellInfo* prevSpellInfo = sSpellMgr->GetSpellInfo(f_spell)->GetPrevRankSpell();
			if ((prevSpellInfo && player->HasSpell(prevSpellInfo->Id)) || !prevSpellInfo) 
				player->LearnSpell(f_spell, false);
		}
	}
};
