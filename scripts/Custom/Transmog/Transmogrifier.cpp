    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            if (sTransmogrification->EnableSets)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/RAIDFRAME/UI-RAIDFRAME-MAINASSIST:30:30:-18:0|tУправлять сетами", EQUIPMENT_SLOT_END + 4, 0);
#endif
            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУбрать всю трансмогрификацию", EQUIPMENT_SLOT_END + 2, 0, "Убрать трансмогрификацию со всех одетых предметов?", 0, false);
            ...
            switch (sender)
            {
                ...
                case EQUIPMENT_SLOT_END + 4: // Presets menu
                {
                    ...
                    if (sTransmogrification->EnableSetInfo)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tКак работают сеты?", EQUIPMENT_SLOT_END + 10, 0);
                ...
                case EQUIPMENT_SLOT_END + 6: // view preset
                {
                    ...
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tИспользовать сет", EQUIPMENT_SLOT_END + 5, action, "Использование этого сета трансмогрификации сделает обращаемые предметы привязанными к вам.\nХотите продолжить?\n\n" + it->second.name, 0, false);
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|tDelete set", EQUIPMENT_SLOT_END + 7, action, "Уверены, что хотите удалить " + it->second.name + "?", 0, false);
                ...
                case EQUIPMENT_SLOT_END + 8: // Save preset
                {
                    ...
                    if (canSave)
                        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tСохранить сет", 0, 0, "Введите имя сета", cost*sTransmogrification->SetCostModifier + sTransmogrification->SetCopperCost, true);
        ...
        bool OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code) override
        {
            ...
            static const char* allowedcharacters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz _.,'1234567890АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя";
        ...
        void ShowTransmogItems(Player* player, Creature* creature, uint8 slot) // Only checks bags while can use an item from anywhere in inventory
        {
            ...
            if (oldItem)
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (sTransmogrification->GetFakeEntry(oldItem) == newItem->GetEntry())
                        continue;
                    ++limit;
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sTransmogrification->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sTransmogrification->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), "Использование этого предмета для трансмогрификации сделает обращаемые предметы привязанными к вам.\nХотите продолжить?\n\n" + sTransmogrification->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sTransmogrification->GetItemLink(newItem, session) + ss.str(), price, false);
                }
            ...
            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУбрать трансмогрификацию", EQUIPMENT_SLOT_END + 3, slot, "Убрать трансмогрификацию этой позиции?", 0, false);
