            ...
            if (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_TD_CHECK_COMBAT:
                    {
                        ...
                        _events.ScheduleEvent(EVENT_TD_CHECK_COMBAT, 1000);
						if (_damageTimes.begin() == _damageTimes.end())
							EnterEvadeMode();
                        break;
                    }
...
public:
	...
	{
		player->ADD_GOSSIP_ITEM_EXTENDED(0, "Я хочу назвать свой Промо-Код.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1, "", 0, true);
	...
	bool OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, char const* code)
	{
		...
		QueryResult SearchForCode = CharacterDatabase.PQuery(""
			//		0		  1				  2			3				4		  5				  6			7		  8
			"SELECT pc.item1, pc.item1_count, pc.item2, pc.item2_count, pc.item3, pc.item3_count, pc.honor, pc.arena, pc.title_id "
			"	FROM promotion_codes as pc "
			"	WHERE pc.code_count > 0 "
			"		and pc.code = %u "
			"		and (SELECT code FROM promotion_codes_used as pcu WHERE pcu.Code = %u "
			"				and IF(pc.account_only = 1, "
			"						IF(pcu.AccountID = %u, 1, 0), "
			"						IF(pcu.PlayerGUID = %u, 1, 0) "
			"					  ) "
			"			) IS NULL; ", codeUINT, codeUINT, player->GetSession()->GetAccountId(), player->GetGUIDLow());
		if (!SearchForCode)
		{
			creature->AI()->Talk(SAY_WRONG);
		}
		else
		{
			...
			do
			{
				...
				/*items start*/
				if (item = fields[0].GetUInt32())
					player->AddItem(item, fields[1].GetUInt32());
				if (item = fields[2].GetUInt32())
					player->AddItem(item, fields[3].GetUInt32());
				if (item = fields[4].GetUInt32())
					player->AddItem(item, fields[5].GetUInt32());
				/*items end.*/
				//Honor
				if (uint32 honor = fields[6].GetUInt32())
					player->ModifyHonorPoints(honor);
				//Arena points
				if (uint32 arena = fields[7].GetUInt32())
					player->ModifyArenaPoints(arena);
				//Titles
				if (uint32 titleid = fields[8].GetUInt32())
				...
				CharacterDatabase.PQuery("UPDATE promotion_codes SET code_count = code_count - 1 WHERE code = %u", codeUINT);
				CharacterDatabase.PQuery("INSERT INTO promotion_codes_used (Code, AccountID, PlayerGUID) VALUES (%u, %u, %u)", 
													codeUINT, player->GetSession()->GetAccountId(), player->GetGUIDLow());
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature) override
	{
		...
		else creature->Whisper("Вы не ВИП!", Language(0), player);
		return false;
	}
};

extern void SetEnchantVisual(Player* player, Item* item, int32 enchant = -1);

class npc_enchanter : public CreatureScript
{
public:
	npc_enchanter() : CreatureScript("npc_v_enchanter") {
		init();
	}

	std::vector<std::string> EnchanterVisualsName;

	void init()
	{
		EnchanterVisualsName.push_back("Берсерк");				//1
		EnchanterVisualsName.push_back("Заряд Сверхновой");		//2
		EnchanterVisualsName.push_back("Светлый лёд");			//3
		EnchanterVisualsName.push_back("Темная материя");		//4
		EnchanterVisualsName.push_back("Поток кровавых паров"); //5
		EnchanterVisualsName.push_back("Заряд Скверны");		//6
		EnchanterVisualsName.push_back("Заряд холода");			//7
		EnchanterVisualsName.push_back("Сила ветра");			//8
		EnchanterVisualsName.push_back("Код отклика \"Бетта\"");//9
		EnchanterVisualsName.push_back("Темный лёд");			//10
		EnchanterVisualsName.push_back("Пульсация берсерка");	//11
		EnchanterVisualsName.push_back("Разряды молний");		//12
		EnchanterVisualsName.push_back("Свет Испепелителя");	//13
		EnchanterVisualsName.push_back("Заряды Сверхновых");	//14
		EnchanterVisualsName.push_back("Мистическая энергия");	//15
		EnchanterVisualsName.push_back("Поток чистого света");	//16
		EnchanterVisualsName.push_back("Яд");					//17
		EnchanterVisualsName.push_back("Мерцание мистицизма");	//18
		EnchanterVisualsName.push_back("Глубокий яд");			//19
		EnchanterVisualsName.push_back("Огни мистицизма");		//20
		EnchanterVisualsName.push_back("Огни чистого света");	//21
		EnchanterVisualsName.push_back("Мерцание чистого света");//22
		EnchanterVisualsName.push_back("Первородный Свет");		//23
		EnchanterVisualsName.push_back("Пульсация крови");		//24
		EnchanterVisualsName.push_back("Сгусток чистого света");//25
		EnchanterVisualsName.push_back("Сила камня");			//26
		EnchanterVisualsName.push_back("Сияние льда");			//27
		EnchanterVisualsName.push_back("Сила льда");			//28
		EnchanterVisualsName.push_back("Сила огня");			//29
		EnchanterVisualsName.push_back("Кровавое мерцание");	//30
		EnchanterVisualsName.push_back("Мерцание Первородного Света");//31
		EnchanterVisualsName.push_back("Безупречная заточка");	//32
		EnchanterVisualsName.push_back("Поток синей материи");	//33
		EnchanterVisualsName.push_back("Капли яда");			//34
		EnchanterVisualsName.push_back("Поток огненной материи");//35
		EnchanterVisualsName.push_back("Заряды синей материи");//36
		EnchanterVisualsName.push_back("Мерцание синей материи");//37
		EnchanterVisualsName.push_back("Холодный огонь");//38
	}

	bool OnGossipHello(Player* player, Creature* creature) override
	{
		player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Я хочу выбрать визуальный эффект зачарования.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
		player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Я хочу рискнуть и наложить случайный визуальный эффект зачарования", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
		player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
		return true;
	}
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
	{
		...
		switch (sender)
		{
			case GOSSIP_SENDER_MAIN:
			{
				...
				{
				case GOSSIP_ACTION_INFO_DEF + 1:
					for (uint8 i = 0; i < EnchanterVisualsName.size(); i++) {
						player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, EnchanterVisualsName[i], GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1 + i);
						if (i == MAX_OPTIONS - 2)
						{
							player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Следующий лист", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
							break;
						}
					}
					break;
				case GOSSIP_ACTION_INFO_DEF + 3:
					for (uint8 i = MAX_OPTIONS - 1; i < EnchanterVisualsName.size(); i++)
						player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, EnchanterVisualsName[i], GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1 + i);
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Предыдущий лист", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
					break;
				case GOSSIP_ACTION_INFO_DEF + 2:
					player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "Хочу наложить случайный визуальный эффект зачарования на правую руку", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + 1, "Вы уверены?", 500 * GOLD, false);
					player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "Хочу наложить случайный визуальный эффект зачарования на левую руку", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + 2, "Вы уверены?", 500 * GOLD, false);
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Назад", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
					break;
				case GOSSIP_ACTION_INFO_DEF:
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Я хочу выбрать визуальный эффект зачарования.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Я хочу рискнуть и наложить случайный визуальный эффект зачарования", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
			...
			case GOSSIP_SENDER_MAIN + 1:
			{
				uint8 enchant_id = action - GOSSIP_ACTION_INFO_DEF - 1;
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "Хочу наложить визуальный эффект \"" + EnchanterVisualsName[enchant_id] + "\" на правую руку", GOSSIP_SENDER_MAIN + 3, GOSSIP_ACTION_INFO_DEF + 1 + enchant_id, "Вы уверены?", 800 * GOLD, false);
				player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "Хочу наложить визуальный эффект \"" + EnchanterVisualsName[enchant_id] + "\" на левую руку", GOSSIP_SENDER_MAIN + 4, GOSSIP_ACTION_INFO_DEF + 1 + enchant_id, "Вы уверены?", 800 * GOLD, false);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, "Назад..", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
				player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
			} break;
			case GOSSIP_SENDER_MAIN + 2:
			{
				...
				case GOSSIP_ACTION_INFO_DEF + 1:
					...
					{
						creature->Whisper("В правой руке нет оружия!", LANG_UNIVERSAL, player);
						player->PlayerTalkClass->SendCloseGossip();
						return true;
					}
					break;
				case GOSSIP_ACTION_INFO_DEF + 2:
					...
					{
						creature->Whisper("В левой руке нет оружия!", LANG_UNIVERSAL, player);
						player->PlayerTalkClass->SendCloseGossip();
						return true;
					}
					break;
				...
				else
				{
					...
					{
					case GOSSIP_ACTION_INFO_DEF + 1:
						SetEnchantVisual(player, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND));
						break;
					case GOSSIP_ACTION_INFO_DEF + 2:
						SetEnchantVisual(player, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND));
						break;
					}
				}
				player->PlayerTalkClass->SendCloseGossip();
			...
			case GOSSIP_SENDER_MAIN + 3:
			{
				...
				{
					if (!player->HasEnoughMoney(800 * GOLD))
						player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, 0, 0, 0);
					else
					{
						player->ModifyMoney(-(800 * GOLD));
						uint8 enchant_id = action - GOSSIP_ACTION_INFO_DEF - 1;
						SetEnchantVisual(player, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND), enchant_id);
						player->PlayerTalkClass->SendCloseGossip();
					}
				}
			} break;
			case GOSSIP_SENDER_MAIN + 4:
			{
				...
				else
				{
					...
					else
					{
						player->ModifyMoney(-(800 * GOLD));
						uint8 enchant_id = action - GOSSIP_ACTION_INFO_DEF - 1;
						SetEnchantVisual(player, player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND), enchant_id);
						player->PlayerTalkClass->SendCloseGossip();
					}
				}
			} break;
...
enum t2_crystal_events
{
	T_EVENT_TELEPORTATION = 1,
	T_EVENT_DEAL_DAMAGE,

};

enum t2_crystal_spells
{
	T_SPELL_SAFE_ZONE = 700088,
	T_SPELL_SAFE_ZONE_V,
	T_AURA_NO_HEAL,
};

class boss_t2_crystal : public CreatureScript
{
public:
	boss_t2_crystal() : CreatureScript("boss_t2_crystal") { }

	struct boss_t2_crystalAI : public ScriptedAI
	{
		boss_t2_crystalAI(Creature* creature) : ScriptedAI(creature)
		{
			Initialize();
			zones.push_back(new Position(-55.899f, 2.115f, 0.631f, 6.2522f));
			zones.push_back(new Position(-31.9281f, -35.98914f, 0.631f, 0.986584f));
			zones.push_back(new Position(10.554647f, -19.860016f, 0.63091f, 2.043390f));
			zones.push_back(new Position(10.478007f, 23.867027f, 0.630892f, 3.569416f));
			zones.push_back(new Position(-31.99172f, 40.072193f, 0.630925f, 4.955642f));
		}
		...
		void ExecuteEvent(uint32 eventId)
		{
			...
			case T_EVENT_TELEPORTATION:
				...
				events.RescheduleEvent(T_EVENT_DEAL_DAMAGE, 14000);
				events.RescheduleEvent(T_EVENT_TELEPORTATION, 35000);
				break;
			case T_EVENT_DEAL_DAMAGE:
				if (player && player->IsAlive() && !player->HasAura(T_SPELL_SAFE_ZONE))
					me->DealDamage(player, player->GetMaxHealth()*0.1f, 0, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW);
				events.RescheduleEvent(T_EVENT_DEAL_DAMAGE, 500);
				break;
			}
		}
	...
	CreatureAI* GetAI(Creature* creature) const override
	{
		return new boss_t2_crystalAI(creature);
	};
};

Position pos[11] = {
	{ -93.428734f, 840.312683f, -28.994547f, 5.719127f }, //1
	{ -56.425861f, 845.138550f, -32.253162f, 0.322654f }, //2
	{ 11.191317f, 861.814758f, -27.199007f, 3.3f }, //3
	{ 20.389736f, 810.831482f, -29.540995f, 0.0f }, //4
	{ -6.041192f, 755.722107f, -28.216816f, 0.0f }, //5
	{ -38.550964f, 775.270996f, -32.595234f, 0.0f }, //6
	{ -78.676918f, 767.953491f, -29.141214f, 0.0f }, //7
	{ -74.452484f, 808.165527f, -32.211414f, 0.0f }, //8
	{ -92.647690f, 862.374817f, -26.557375f, 0.0f }, //9
	{ -31.540659f, 830.707642f, -29.535845f, 0.0f }, //10
	{ -38.5f, 812.8f, -28.535791f, 0.0f }, //11
};

enum t3_actions_and_events
{
	TTHREE_ACTION_START_EVENT = 1,
	TTHREE_ACTION_STOP_EVENT,
	TTHREE_ACTION_UPDATE_COUNTER,

	TTHREE_EVENT_TELEPORT = 1,
	TTHREE_EVENT_SPAWN_ZOMBIE,
	TTHREE_EVENT_DO_FIRE,
};
...
{
public:
	boss_t3_fire() : CreatureScript("boss_t3_fire") { }

	struct boss_t3_fireAI : public ScriptedAI
	{
		boss_t3_fireAI(Creature* creature) : ScriptedAI(creature)
		{
			Initialize();
		}
		...
		void DoAction(int32 action) override
		{
			switch (action)
			{
			case TTHREE_ACTION_START_EVENT:
				me->SetDisplayId(11686);
				me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
				events.ScheduleEvent(TTHREE_EVENT_TELEPORT, 40000);
				events.ScheduleEvent(TTHREE_EVENT_SPAWN_ZOMBIE, 5000);
				events.ScheduleEvent(TTHREE_EVENT_DO_FIRE, 2000);
			...
			case TTHREE_ACTION_STOP_EVENT:
				me->DeMorph();
				me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
				events.CancelEvent(TTHREE_EVENT_TELEPORT);
				events.CancelEvent(TTHREE_EVENT_DO_FIRE);
				events.CancelEvent(TTHREE_EVENT_SPAWN_ZOMBIE);
				me->CombatStop();
				player->CombatStop();
				me->ClearInCombat();
		...
		void RemoveAllZombie()
		{
			...
			Trinity::AllCreaturesOfEntryInRange u_check(me, 60027, 200.0f);
			Trinity::UnitListSearcher<Trinity::AllCreaturesOfEntryInRange> searcher(me, targets, u_check);
			me->VisitNearbyObject(200.0f, searcher);
			for (std::list<Unit*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
			{
				Unit* un = (*iter);
				me->DealDamage(un, un->GetMaxHealth());
			}
		}
		...
		void SummonedCreatureDespawn(Creature* /*summon*/) override 
		{
			...
			if (counter >= 9)
				DoAction(TTHREE_ACTION_STOP_EVENT);
		}
		...
		void ExecuteEvent(uint32 eventId)
		{
			...
			switch (eventId)
			{
				case TTHREE_EVENT_TELEPORT:
				{	
					uint8 u = urand(0, 9);
					while (u == zone)
						u = urand(0, 9);
					me->NearTeleportTo(pos[u].GetPositionX(), pos[u].GetPositionY(), pos[u].GetPositionZ(), pos[u].GetOrientation());
					events.RescheduleEvent(TTHREE_EVENT_TELEPORT, 40000);
					break; 
				}
				case TTHREE_EVENT_SPAWN_ZOMBIE:
				{
					if (z_counter++ < 9) {
						TempSummon* cre = me->SummonCreature(60027, pos[10].GetPosition(), TEMPSUMMON_CORPSE_DESPAWN);
						me->AddAura(TTHREE_AURA_INVINSIBLE, cre);
						cre->Attack(player, true);
						cre->GetMotionMaster()->MoveFollow(player, 1, 0);
						events.RescheduleEvent(TTHREE_EVENT_SPAWN_ZOMBIE, 30000);
					}
					break;
				}
				case TTHREE_EVENT_DO_FIRE:
				{
					Trinity::AnyUnitInObjectRangeCheck u_check(me, 3.0f);
					Trinity::UnitListSearcher<Trinity::AnyUnitInObjectRangeCheck> searcher(me, targets, u_check);
					me->VisitNearbyObject(3.0f, searcher);
					for (std::list<Unit*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
					{
						Unit* un = (*iter);
						if (un->GetTypeId() == TYPEID_PLAYER && un->ToPlayer() == player)
							me->Kill(player, false);
						else if (un->GetTypeId() == TYPEID_UNIT)
							me->DealDamage(un, 10);
					}
					events.RescheduleEvent(TTHREE_EVENT_DO_FIRE, 1000);
					break;
				}
			}
		}
	...
	bool OnGossipHello(Player* player, Creature* creature) override
	{
		creature->GetAI()->DoAction(TTHREE_ACTION_START_EVENT);
	...
	CreatureAI* GetAI(Creature* creature) const override
	{
		return new boss_t3_fireAI(creature);
	};
};
