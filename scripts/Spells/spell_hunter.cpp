...
enum sacrifice_cooldown_spells {
	SPELL_PET_HEART_OF_PHOENIX = 55709,
	SPELL_HUN_PET_RESSURECTION = 982,
};

class spell_hun_sacrifice : public SpellScriptLoader
{
public:
	spell_hun_sacrifice() : SpellScriptLoader("spell_hun_sacrifice") { }

	class spell_hun_sacrifice_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_hun_sacrifice_SpellScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_HUNTER_SACRIFICE))
				return false;
			return true;
		}
		...
		void HandleDummy(SpellEffIndex /*effIndex*/)
		{
			RecoveryTime = sSpellMgr->GetSpellInfo(SPELL_HUNTER_SACRIFICE)->RecoveryTime;
			Player* caster = GetCaster()->ToPlayer();
			Pet* pet = caster->GetPet();
			WorldPacket data;

			caster->HealBySpell(caster, sSpellMgr->GetSpellInfo(SPELL_HUNTER_SACRIFICE), caster->GetPet()->GetHealth()*uint32(GetEffectValue()) / 100);
			
			caster->Kill(pet);
			if (pet->HasSpell(SPELL_PET_HEART_OF_PHOENIX))
				if (pet->GetSpellHistory()->GetRemainingCooldown(SPELL_PET_HEART_OF_PHOENIX) < RecoveryTime)
				{
					pet->GetSpellHistory()->ResetCooldown(SPELL_PET_HEART_OF_PHOENIX, false);
					pet->GetSpellHistory()->AddCooldown(SPELL_PET_HEART_OF_PHOENIX, 0, std::chrono::milliseconds(RecoveryTime));
					
					pet->GetSpellHistory()->BuildCooldownPacket(data, SPELL_COOLDOWN_FLAG_NONE, SPELL_PET_HEART_OF_PHOENIX, RecoveryTime);
					caster->SendDirectMessage(&data);
				}
			
			caster->GetSpellHistory()->AddCooldown(SPELL_HUN_PET_RESSURECTION, 0, std::chrono::milliseconds(RecoveryTime));
			
			caster->GetSpellHistory()->BuildCooldownPacket(data, SPELL_COOLDOWN_FLAG_NONE, SPELL_HUN_PET_RESSURECTION, RecoveryTime);
			caster->SendDirectMessage(&data);
		}
		...
		SpellCastResult CheckCast()
		{
			...
			if (!(caster->GetPetGUID()) || caster->GetPet()->isDead())
		...
		void Register() override
		{
			OnCheckCast += SpellCheckCastFn(spell_hun_sacrifice_SpellScript::CheckCast);
			OnEffectHitTarget += SpellEffectFn(spell_hun_sacrifice_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
		}
	private:
		uint32 RecoveryTime;
	};
	...
	SpellScript* GetSpellScript() const override
	{
		return new spell_hun_sacrifice_SpellScript();
	}
};
