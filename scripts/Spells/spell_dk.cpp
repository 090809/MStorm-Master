    ...
    public:
        ...
        private:
            ...
            void Trigger(AuraEffect* aurEff, DamageInfo& /*dmgInfo*/, uint32& absorbAmount)
            {
                ...
                int32 bp = CalculatePct(absorbAmount, 0.01f);
    ...
    public:
        ...
        private:
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, int32 & amount, bool & /*canBeRecalculated*/)
            {
				...
				else if (Player* player = GetCaster()->GetCharmerOrOwner()->ToPlayer())
					amount += int32(2 * player->GetTotalAttackPowerValue(BASE_ATTACK));
            }
...
class spell_dk_death_grip : public SpellScriptLoader
{
    public:
        ...
        class spell_dk_death_grip_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                int32 damage = GetEffectValue(); //49575
        ...
        class spell_dk_death_strike_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    int32 bp = int32(count * caster->CountPctFromMaxHealth(int32(GetSpellInfo()->Effects[EFFECT_0].DamageMultiplier)) * 0.9f);
    ...
    public:
        ...
        private:
            ...
            void HandleRaiseDead(SpellEffIndex /*effIndex*/)
            {
				...
				GetCaster()->CastSpell(GetCaster(), GetGhoulSpellId(), TriggerCastFlags(TRIGGERED_FULL_MASK));
			}
...
// 200300 - Blood power
class spell_dk_blood_power : public SpellScriptLoader
{
public:
	spell_dk_blood_power() : SpellScriptLoader("spell_dk_blood_power") { }

	class spell_dk_blood_power_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_dk_blood_power_SpellScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200300))
				return false;
			return true;
		}

		float need_health_in_pct = 15.0f;
		int32 health;
		...
		SpellCastResult CheckCast()
		{
			...
			if (caster->HasAura(200301)) //Улучшенный кровавый импульс 1 уровень
				need_health_in_pct += 5.0f;
			if (caster->HasAura(200302)) //Улучшенный кровавый импульс 2 уровень
				need_health_in_pct += 10.0f;
			if (caster->HasAura(200303)) //Улучшенный кровавый импульс 3 уровень
				need_health_in_pct += 15.0f;

			if (caster->GetHealthPct() <= need_health_in_pct)
		...
		void HandleDamage()
		{
			health = GetCaster()->CountPctFromMaxHealth(need_health_in_pct);
			SetHitDamage(0);
			...
			SpellNonMeleeDamage damageInfo_2(GetCaster(), GetCaster(), GetSpellInfo()->Id, GetSpellInfo()->SchoolMask);
			damageInfo_2.damage = health;
			GetCaster()->SendSpellNonMeleeDamageLog(&damageInfo_2);
			GetCaster()->DealSpellDamage(&damageInfo_2, false);
		}
		...
		void HandleAfterHit()
		{
			...
			{
				caster->HealBySpell(caster, GetSpellInfo(), health);
			}
		}

		void Register() override
		{
			OnCheckCast += SpellCheckCastFn(spell_dk_blood_power_SpellScript::CheckCast);
			OnHit += SpellHitFn(spell_dk_blood_power_SpellScript::HandleDamage);
			AfterHit += SpellHitFn(spell_dk_blood_power_SpellScript::HandleAfterHit);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_dk_blood_power_SpellScript();
	}
};

class spell_dk_mirror : public SpellScriptLoader     // 200304
{
public:
	spell_dk_mirror() : SpellScriptLoader("spell_dk_mirror") { }

	class spell_dk_mirror_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_dk_mirror_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200304))
				return false;
			return true;
		}
		...
		{
			int pct = 20;
			if (GetCaster()->HasAura(200305)) pct += 10;
			if (GetCaster()->HasAura(200306)) pct += 20;
			if (GetCaster()->HasAura(200307)) pct += 30;

			int32 damage = CalculatePct(int32(eventInfo.GetDamageInfo()->GetDamage()), pct);
			GetTarget()->CastCustomSpell(64039, SPELLVALUE_BASE_POINT0, damage, eventInfo.GetDamageInfo()->GetAttacker(), true, NULL, aurEff);
		}

		void Register() override
		{
			OnEffectProc += AuraEffectProcFn(spell_dk_mirror_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_dk_mirror_AuraScript();
	}
};
