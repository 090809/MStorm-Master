...
class spell_item_15_flascs_drops : public SpellScriptLoader
{
public:
	spell_item_15_flascs_drops() : SpellScriptLoader("spell_item_15_flascs_drops") { }

	class spell_item_15_flascs_drops_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_item_15_flascs_drops_SpellScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(35874) //Melee 
			 || !sSpellMgr->GetSpellInfo(35912) //SPD 
		     || !sSpellMgr->GetSpellInfo(38734)) //Ranged
				return false;
			return true;
		}

		SpellCastResult CheckProc()
		{
			Unit* caster = GetCaster();
			if (caster->HasAura(35874)) caster->RemoveAura(35874);
			if (caster->HasAura(35912)) caster->RemoveAura(35912);
			if (caster->HasAura(38734)) caster->RemoveAura(38734);
			return SPELL_CAST_OK;
		}

		void Register() override
		{
			OnCheckCast += SpellCheckCastFn(spell_item_15_flascs_drops_SpellScript::CheckProc);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_item_15_flascs_drops_SpellScript();
	}
};

class spell_item_sharp : public SpellScriptLoader 
{
public: 
	spell_item_sharp() : SpellScriptLoader("spell_item_sharp_visual") { }
		class spell_item_sharp_spellscript : public SpellScript
		{
			PrepareSpellScript(spell_item_sharp_spellscript);
			bool Validate(SpellInfo const* /*spellInfo*/) override
			{
				if (!sSpellMgr->GetSpellInfo(100101)) 
					return false;
				return true;
			}
			...
			void HandleDummy()
			{
				...
				switch (CastItemEntry)
				{
				case 527000: if (Chance < 30) { ChangeItem = true; } break;
				case 527001: if (Chance < 45) { ChangeItem = true; } break;
				case 527002: if (Chance < 60) { ChangeItem = true; } break;
				case 527003: if (Chance < 75) { ChangeItem = true; } break;
				} 
				...
				if (ChangeItem)  { 
					...
					TarItem->GetOwner()->Say(text_bad, LANG_UNIVERSAL);
					TarItem->GetOwner()->HandleEmoteCommand(EMOTE_ONESHOT_CRY);
					TarItem->GetOwner()->DestroyItemCount(TarItem->GetEntry(), 1, true);
				};
			}

			void Register() override {
				OnCheckCast += SpellCheckCastFn(spell_item_sharp_spellscript::CheckCast);
				AfterCast += SpellCastFn(spell_item_sharp_spellscript::HandleDummy);
			}
		private:
			Item* TarItem, *CastItem;
			uint32 new_entry, CastItemEntry;

		};

		SpellScript* GetSpellScript() const override
		{
			return new spell_item_sharp_spellscript();
		}
};

class spell_item_timelaps : public SpellScriptLoader
{
public:
	spell_item_timelaps() : SpellScriptLoader("spell_item_timelaps") { }
	class spell_item_timelaps_spellscript : public SpellScript
	{
		PrepareSpellScript(spell_item_timelaps_spellscript);
		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(100200))
				return false;
			return true;
		}
		...
		SpellCastResult CheckCast()
		{
			...
			if (!count)
			{
				ChatHandler(player->GetSession()).PSendSysMessage("У Вас нет закрепленных копий подземелий и рейдов. Предмет не был использован.");
		...
		void HandleDummy()
		{
			...
			ChatHandler(player->GetSession()).PSendSysMessage("Ваши копии подземелий и рейдов обнолены.");
		}

		void Register() override {
			OnCheckCast += SpellCheckCastFn(spell_item_timelaps_spellscript::CheckCast);
			AfterCast += SpellCastFn(spell_item_timelaps_spellscript::HandleDummy);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_item_timelaps_spellscript();
	}
};

class spell_item_golden_plag : public SpellScriptLoader
{
public:
	spell_item_golden_plag() : SpellScriptLoader("spell_item_golden_plag") { }

	class spell_item_golden_plag_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_item_golden_plag_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(100300))
				return false;
			return true;
		}
		...
		void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
		{
			if (eventInfo.GetActionTarget()->ToCreature()->GetEntry() == 553601)
				GetCaster()->ToPlayer()->ModifyMoney(GOLD * sWorld->getIntConfig(CONFIG_GOLD_PLAG_GOLD_COUNT));
		}

		void Register() override
		{
			OnEffectProc += AuraEffectProcFn(spell_item_golden_plag_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_item_golden_plag_AuraScript();
	}
};

class spell_item_arena_points : public SpellScriptLoader
{
public:
	spell_item_arena_points() : SpellScriptLoader("spell_item_arena_points") { }
	class spell_item_arena_points_spellscript : public SpellScript
	{
		PrepareSpellScript(spell_item_arena_points_spellscript);
		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(800001))
				return false;
			return true;
		}
		...
		{
			Player* player = GetCaster()->ToPlayer();
			player->ModifyArenaPoints(100); //Захаркоденно, в крайнем случае допилить.
		}

		void Register() override {
			AfterCast += SpellCastFn(spell_item_arena_points_spellscript::HandleDummy);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_item_arena_points_spellscript();
	}
};

class spell_item_froustmourne : public SpellScriptLoader
{
public:
	spell_item_froustmourne() : SpellScriptLoader("spell_item_froustmourne") { }

	class spell_item_froustmourne_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_item_froustmourne_SpellScript);

		bool Validate(SpellInfo const* /*spell*/) override
		{
			if (!sSpellMgr->GetSpellInfo(610003) || !sSpellMgr->GetSpellInfo(610004))
				return false;
			return true;
		}
		...
		{
			Unit* caster = GetCaster();
			Aura* aura = caster->GetAura(610004); //Жатва
			if (!aura) //Если нет жатвы
			{
				aura = caster->GetAura(610003); //Длани
				if (aura->GetStackAmount() >= 4 && !caster->GetSpellHistory()->HasCooldown(610004))
					//Если дланей 5 и Жатва не имеет кулдауна, тогда снимаем все ауры дланей, кастуем жатву
					//и добавляем на нее кулдаун.
				{
					aura->Remove();
					caster->CastSpell(caster, 610004, true);
					caster->GetSpellHistory()->AddCooldown(610004, 0, std::chrono::seconds(60)); //
				}
			}
		}
		...
		void Register() override
		{
			OnEffectHitTarget += SpellEffectFn(spell_item_froustmourne_SpellScript::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_item_froustmourne_SpellScript();
	}
};



class spell_item_leg_pike : public SpellScriptLoader
{
public:
	spell_item_leg_pike() : SpellScriptLoader("spell_item_leg_pike") { }

	class spell_item_leg_pike_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_item_leg_pike_AuraScript);

		bool Validate(SpellInfo const* /*spell*/) override
		{
			if (!sSpellMgr->GetSpellInfo(610007) || !sSpellMgr->GetSpellInfo(610008))
				return false;
			return true;
		}
		...
		void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
		{
			int32 damage = eventInfo.GetDamageInfo()->GetDamage() * 0.25f;
			Unit* target = eventInfo.GetActionTarget();
			Unit* caster = eventInfo.GetActor();
			if (caster && target && damage > 0)
				caster->CastCustomSpell(target, 610008, &damage, 0, 0, true);
		}

		void Register() override
		{
			OnEffectProc += AuraEffectProcFn(spell_item_leg_pike_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_item_leg_pike_AuraScript();
	}
};
...
void AddSC_item_spell_scripts()
{
	...
	new spell_item_15_flascs_drops();

	new spell_item_sharp();
	new spell_item_timelaps();
	new spell_item_golden_plag();

	new spell_item_arena_points();
