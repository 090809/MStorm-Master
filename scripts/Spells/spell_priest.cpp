...
enum PriestSpells
{
	...
	SPELL_PRIEST_DIVINE_THREAD						= 200517,
};
...
enum divine_thread_spells {
	SPELL_PRIEST_POWER_WORD_SHIELD = 17,
	SPELL_PRIEST_RENEW			   = 139,
};

// 200517 - SPELL_PRIEST_DIVINE_THREAD 
class spell_pri_divine_thread : public SpellScriptLoader
{
public:
	spell_pri_divine_thread() : SpellScriptLoader("spell_pri_divine_thread") { }

	class spell_pri_divine_thread_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_pri_divine_thread_SpellScript);

		SpellCastResult CheckCast()
		{
			//if (GetCaster() && GetExplTargetUnit() && GetCaster() == GetExplTargetUnit()); //Невозможно использовать на себя!
			//	return SPELL_FAILED_BAD_TARGETS;
		...
		void HandleAfterHit()
		{
			if (Unit* unitTarget = GetHitUnit())
			{
				unitTarget->NearTeleportTo(GetCaster()->GetPositionX(), GetCaster()->GetPositionY(), GetCaster()->GetPositionZ(), unitTarget->GetOrientation());
				GetCaster()->CastSpell(unitTarget, sSpellMgr->GetLastSpellInChain(SPELL_PRIEST_POWER_WORD_SHIELD), tr_flag);
				GetCaster()->CastSpell(unitTarget, sSpellMgr->GetLastSpellInChain(SPELL_PRIEST_RENEW), tr_flag);
			}
		...
		void Register() override 
		{
			OnCheckCast += SpellCheckCastFn(spell_pri_divine_thread_SpellScript::CheckCast);
			AfterHit += SpellHitFn(spell_pri_divine_thread_SpellScript::HandleAfterHit);
		}

	private:
		TriggerCastFlags tr_flag = TriggerCastFlags(TRIGGERED_IGNORE_GCD
	...
	SpellScript* GetSpellScript() const override
	{
		return new spell_pri_divine_thread_SpellScript;
	}
};
