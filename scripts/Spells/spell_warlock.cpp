...
enum WarlockSpells
{
	...
	SPELL_WARLOCK_PET_SEDUCTION						= 6358,
	SPELL_WARLOCK_GLYPH_OF_SUCCUBUSS				= 56250,
	SPELL_WARLOCK_TALENT_IMPROWED_SUCCUBUS_1		= 18754,
	SPELL_WARLOCK_TALENT_IMPROWED_SUCCUBUS_2		= 18755,
	SPELL_WARLOCK_TALENT_IMPROWED_SUCCUBUS_3		= 18756,
};
...
class spell_warl_pet_seduction : public SpellScriptLoader
{
public:
	spell_warl_pet_seduction() : SpellScriptLoader("spell_warl_pet_seduction") { }

	class spell_warl_pet_seduction_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_warl_pet_seduction_SpellScript);

		SpellCastResult ReduceCastTime() {
			int32 time = 1500;
			if (GetCaster()->GetOwner()->HasSpell(SPELL_WARLOCK_TALENT_IMPROWED_SUCCUBUS_1)) time = 1170;
			if (GetCaster()->GetOwner()->HasSpell(SPELL_WARLOCK_TALENT_IMPROWED_SUCCUBUS_2)) time = 810;
			if (GetCaster()->GetOwner()->HasSpell(SPELL_WARLOCK_TALENT_IMPROWED_SUCCUBUS_3)) time = 500;
			GetCaster()->ModSpellCastTime(GetSpellInfo(), time, GetSpell());
			return SPELL_CAST_OK;
		}

		void RemoveAuras(SpellEffIndex /**/) {

			Unit* caster = GetCaster();
			if (Unit* target = GetHitUnit())
			{
				if (caster->GetOwner() && caster->GetOwner()->HasAura(SPELL_WARLOCK_GLYPH_OF_SUCCUBUSS))
				{
	...
	private:
		...
		{
			OnCheckCast += SpellCheckCastFn(spell_warl_pet_seduction_SpellScript::ReduceCastTime);
			OnEffectHitTarget += SpellEffectFn(spell_warl_pet_seduction_SpellScript::RemoveAuras, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
		}
	...
	{
		return new spell_warl_pet_seduction_SpellScript();
	}
};

// -7812 - Sacrifice
class spell_warl_pet_sacrifice : public SpellScriptLoader
{
public:
	spell_warl_pet_sacrifice() : SpellScriptLoader("spell_warl_pet_sacrifice") { }

	class spell_warl_pet_sacrifice_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_warl_pet_sacrifice_AuraScript);
		...
		void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& canBeRecalculated)
		{
			...
			{
				float bonus = caster->GetCharmerOrOwner()->GetMaxHealth() * urand(15, 20) / 100;
				amount += int32(bonus);
			}
		}
		...
		void Register() override
		{
			DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_sacrifice_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_warl_pet_sacrifice_AuraScript();
	}
};
            ...
            void HandleTeleport(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (Player* player = GetTarget()->ToPlayer())
                {
                    if (GameObject* circle = player->GetGameObject(SPELL_WARLOCK_DEMONIC_CIRCLE_SUMMON))
                    {
						...
						if (player->HasAura(600007))
							player->AddAura(600008, player);
                    }
                }
            }
...
class spell_warl_negative_barrier : public SpellScriptLoader
{
public:
	spell_warl_negative_barrier() : SpellScriptLoader("spell_warl_negative_barrier") { }

	class spell_warl_negative_barrier_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_warl_negative_barrier_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200923))
				return false;
			return true;
		}
		...
		void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
		{
			...
			if (GetTarget() != healer)
				GetCaster()->DealDamage(healer, heal, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, GetSpellInfo());
		}

		void Register() override
		{
			DoCheckProc += AuraCheckProcFn(spell_warl_negative_barrier_AuraScript::CheckProc);
			OnEffectProc += AuraEffectProcFn(spell_warl_negative_barrier_AuraScript::OnProc, EFFECT_2, SPELL_AURA_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_warl_negative_barrier_AuraScript();
	}
};

class spell_warl_hunter_pre : public SpellScriptLoader
{
public:
	spell_warl_hunter_pre() : SpellScriptLoader("spell_warl_hunter_pre") { }

	class spell_warl_hunter_pre_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_warl_hunter_pre_SpellScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200932))
				return false;
			return true;
		}
			...
			if (caster)
				for (Unit::ControlList::const_iterator itr = caster->m_Controlled.begin(); itr != caster->m_Controlled.end(); ++itr)
					if (Creature* Pet = (*itr)->ToCreature())
						if (Pet->IsAlive() &&
							Pet->GetOwnerGUID() == caster->GetGUID() &&
							Pet->IsWithinDist(caster, 100.0f, false))
						{
							caster->DealDamage(Pet, Pet->GetMaxHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, GetSpellInfo());
							return SPELL_CAST_OK;
						}

			caster->DealDamage(caster, uint32(caster->GetHealth() * 0.7f), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, GetSpellInfo());
			return SPELL_CAST_OK;
		}

		void Register() override
		{
			OnCheckCast += SpellCheckCastFn(spell_warl_hunter_pre_SpellScript::CheckCast);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_warl_hunter_pre_SpellScript();
	}
};

class spell_warl_hunter : public SpellScriptLoader
{
public:
	spell_warl_hunter() : SpellScriptLoader("spell_warl_hunter") { }

	class spell_warl_hunter_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_warl_hunter_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200933))
				return false;
			return true;
		}
		...
		{
			PreventDefaultAction();
			if (eventInfo.GetDamageInfo()->GetVictim()->IsCharmedOwnedByPlayerOrPlayer())
				GetCaster()->DealDamage(GetCaster(), uint32(eventInfo.GetDamageInfo()->GetDamage() * 0.8f), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, GetSpellInfo());
		}

		void Register() override
		{
			DoCheckProc += AuraCheckProcFn(spell_warl_hunter_AuraScript::CheckProc);
			OnEffectProc += AuraEffectProcFn(spell_warl_hunter_AuraScript::OnProc, EFFECT_1, SPELL_AURA_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_warl_hunter_AuraScript();
	}
};

class spell_warl_pet_spell_lock : public SpellScriptLoader
{
public:
	spell_warl_pet_spell_lock() : SpellScriptLoader("spell_warl_pet_spell_lock") { }

	class spell_warl_pet_spell_lock_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_warl_pet_spell_lock_SpellScript);

		void ApplyMute() {

			Unit* caster = GetCaster();
			if (Unit* target = GetHitUnit())
				caster->CastSpell(target, 24259, true);
		}

		void Register() override
		{
			AfterHit += SpellHitFn(spell_warl_pet_spell_lock_SpellScript::ApplyMute);
		}
	...
	SpellScript* GetSpellScript() const override
	{
		return new spell_warl_pet_spell_lock_SpellScript();
	}
};
...
void AddSC_warlock_spell_scripts()
{
	...
	new spell_warl_pet_seduction();
	new spell_warl_pet_sacrifice();

	new spell_warl_negative_barrier();
	new spell_warl_hunter();
	new spell_warl_hunter_pre();
	new spell_warl_pet_spell_lock();
}
