...
enum PaladinSpells
{
	...
	SPELL_PALADIN_HIGHLIHGT = 200042,

	SPELL_MARK_OF_THE_SINNER = 200026,
	SPELL_MARK_OF_THE_SINNER_APR = 200037,

	SPELL_PALADIN_GREATION = 200047
};
	...
	class spell_pal_seal_of_righteousness_AuraScript : public AuraScript
	{
		...
		bool CheckProc(ProcEventInfo& eventInfo)
		{
			if (eventInfo.GetDamageInfo()->GetSpellInfo() 
			 && eventInfo.GetDamageInfo()->GetSpellInfo()->Id == 200028)
				return false;
...
// 20167 - Holy Sigil Proc
class spell_pal_consecration_talents : public SpellScriptLoader
{
public:
	spell_pal_consecration_talents() : SpellScriptLoader("spell_pal_consecration_talents") { }

	class spell_pal_consecration_talentsSpellScript : public SpellScript
	{
		PrepareSpellScript(spell_pal_consecration_talentsSpellScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(48819)
				&& !sSpellMgr->GetSpellInfo(48818)
				&& !sSpellMgr->GetSpellInfo(27173)
				&& !sSpellMgr->GetSpellInfo(26573)
				&& !sSpellMgr->GetSpellInfo(20924)
				&& !sSpellMgr->GetSpellInfo(20923)
				&& !sSpellMgr->GetSpellInfo(20922)
				&& !sSpellMgr->GetSpellInfo(20116))
				return false;
			return true;
		}
		...
		void CheckTalents()
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->HasAura(200001)) //»сцел€ющее осв€щение
					caster->CastSpell(caster, 200000, true);
				
				if (caster->HasAura(200011))  {
					caster->CastSpell(caster, 200010, true); //”сил€ющее осв€щение ур5
				} else if (caster->HasAura(200009))  {
					caster->CastSpell(caster, 200008, true); //”сил€ющее осв€щение ур4
				} else if (caster->HasAura(200007))  {
					caster->CastSpell(caster, 200006, true); //”сил€ющее осв€щение ур3
				} else if (caster->HasAura(200005))  {
					caster->CastSpell(caster, 200004, true); //”сил€ющее осв€щение ур2
				} else if (caster->HasAura(200003))  {
					caster->CastSpell(caster, 200002, true); //”сил€ющее осв€щение ур1
				}
			}
		}
		...
		void Register() override
		{
			AfterCast += SpellCastFn(spell_pal_consecration_talentsSpellScript::CheckTalents);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_pal_consecration_talentsSpellScript();
	}
};

class spell_pal_mark_of_sinner : public SpellScriptLoader
{
public:
	spell_pal_mark_of_sinner() : SpellScriptLoader("spell_pal_mark_of_sinner") { }
	
	class spell_pal_mark_of_sinnerAuraScript : public AuraScript
	{
		PrepareAuraScript(spell_pal_mark_of_sinnerAuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_MARK_OF_THE_SINNER))
				return false;
			return true;
		};
		...
		void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo) {
			PreventDefaultAction();
			if (eventInfo.GetDamageInfo()->GetSpellInfo() && eventInfo.GetDamageInfo()->GetSpellInfo()->Id == 200028)
				return;

			Unit* caster = eventInfo.GetActor();
			float mod = (caster->GetAuraOfRankedSpell(SPELL_MARK_OF_THE_SINNER_APR) != NULL)
				? float(int32(caster->GetAuraOfRankedSpell(SPELL_MARK_OF_THE_SINNER_APR)->GetId()) % SPELL_MARK_OF_THE_SINNER_APR) / 10 + 0.1
				: 0;

			int32 damage = eventInfo.GetDamageInfo()->GetDamage();

			if (damage > 0)
				damage = int32(damage * (0.3f + mod));
			
			Unit* ex_tar = ex_GetTarget(caster);

			if (damage > 0 && ex_tar != NULL)
				caster->CastCustomSpell(ex_tar, 200028, &damage, NULL, NULL, true);
		};

		Unit* ex_GetTarget(Unit* caster) {
			std::list<Unit*> targets;
			Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(caster, caster, 50.0f);
			...
			for (std::list<Unit*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
				if ((*iter)->HasAura(SPELL_MARK_OF_THE_SINNER, caster->GetGUID()) && (*iter) != caster)
				{
					return (*iter);
				}
			return NULL;
		...
		void Register() override
		{
			OnEffectProc += AuraEffectProcFn(spell_pal_mark_of_sinnerAuraScript::HandleProc, EFFECT_1, SPELL_AURA_DUMMY);
		};
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_pal_mark_of_sinnerAuraScript();
	};
};

//SPELL_PALADIN_HIGHLIHGT
class spell_pal_highlight : public SpellScriptLoader
{
public:
	spell_pal_highlight() : SpellScriptLoader("spell_pal_highlight") { }

	class spell_pal_highlight_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_pal_highlight_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_PALADIN_HIGHLIHGT)
				|| !sSpellMgr->GetSpellInfo(SPELL_PALADIN_HIGHLIHGT + 1))
				return false;
			return true;
		}
		...
		{
			if (GetDuration())
				GetCaster()->AddAura(SPELL_PALADIN_HIGHLIHGT + 1, GetTarget());
		}

		void Register() override
		{
			AfterEffectRemove += AuraEffectRemoveFn(spell_pal_highlight_AuraScript::HandleRemove, EFFECT_0, SPELL_AURA_MOD_DECREASE_SPEED, AURA_EFFECT_HANDLE_REAL);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_pal_highlight_AuraScript();
	}
};

//¬еликолепие
class spell_pal_greation : public SpellScriptLoader
{
public:
	spell_pal_greation() : SpellScriptLoader("spell_pal_greation") { }

	class spell_pal_greation_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_pal_greation_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_PALADIN_GREATION))
				return false;
			return true;
		}
		...
		{
			GetTarget()->UpdateMaxPower(POWER_MANA);
		}

		void Register() override
		{
			AfterEffectApply += AuraEffectApplyFn(spell_pal_greation_AuraScript::AfterApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_pal_greation_AuraScript();
	}
};
void AddSC_paladin_spell_scripts()
{
	...
	new spell_pal_consecration_talents();
	new spell_pal_highlight();
	new spell_pal_mark_of_sinner();
	new spell_pal_greation();
}
