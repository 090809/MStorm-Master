...
enum WarriorSpells
{
	...
	SPELL_WARRIOR_PHYS_RESON						= 200137
};
        ...
        class spell_warr_execute_SpellScript : public SpellScript
        {
            ...
            void HandleEffect(SpellEffIndex effIndex)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    int32 bp = GetEffectValue() + int32(rageUsed * spellInfo->Effects[effIndex].DamageMultiplier + caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.5f);
...
// -200123 - Double Reflection
class spell_warrior_double_reflection : public SpellScriptLoader
{
public:
	spell_warrior_double_reflection() : SpellScriptLoader("spell_warrior_double_reflection") { }

	class spell_warrior_double_reflection_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_warrior_double_reflection_AuraScript);

	public:
		spell_warrior_double_reflection_AuraScript()
		{
			absorbChance = 0;
		}
	...
	private:
		...
		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200126))
				return false;
			return true;
		}
		...
		void Register() override
		{
			DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warrior_double_reflection_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
			OnEffectAbsorb += AuraEffectAbsorbFn(spell_warrior_double_reflection_AuraScript::Absorb, EFFECT_0);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_warrior_double_reflection_AuraScript();
	}
};

class spell_war_phys_reson : public SpellScriptLoader
{
public:
	spell_war_phys_reson() : SpellScriptLoader("spell_war_phys_reson") { }

	class spell_war_phys_reson_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_war_phys_reson_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_WARRIOR_PHYS_RESON)
				&& !sSpellMgr->GetSpellInfo(SPELL_WARRIOR_PHYS_RESON + 1)
				&& !sSpellMgr->GetSpellInfo(SPELL_WARRIOR_PHYS_RESON + 2))
				return false;
			return true;
		}
		...
		void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
		{
			if (!eventInfo.GetDamageInfo()->GetBlock()) return;
			uint32 healcount = uint32(eventInfo.GetDamageInfo()->GetBlock() * GetSpellInfo()->Effects[EFFECT_0].CalcValue(GetCaster()) / 100);
			GetCaster()->HealBySpell(GetCaster(), GetSpellInfo(), healcount);
		}

		void Register() override
		{
			OnEffectProc += AuraEffectProcFn(spell_war_phys_reson_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_war_phys_reson_AuraScript();
	}
};

class spell_warr_wanna_life : public SpellScriptLoader
{
public:
	spell_warr_wanna_life() : SpellScriptLoader("spell_warr_wanna_life") { }

	class spell_warr_wanna_life_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_warr_wanna_life_SpellScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200128)				//1
				&& !sSpellMgr->GetSpellInfo(200128 + 2)			//2
				&& !sSpellMgr->GetSpellInfo(200128 + 4)			//3
				&& !sSpellMgr->GetSpellInfo(200128 + 6)			//4
				&& !sSpellMgr->GetSpellInfo(200128 + 8))		//5
				return false;
			return true;
		}
	...
	SpellScript* GetSpellScript() const override
	{
		return new spell_warr_wanna_life_SpellScript();
	}
};
...
void AddSC_warrior_spell_scripts()
{
	...
	new spell_warrior_double_reflection();
	new spell_warr_wanna_life();
	new spell_war_phys_reson();
}
