...
class spell_creature_blood : public SpellScriptLoader
{
public:
	spell_creature_blood() : SpellScriptLoader("spell_creature_blood") { }

	class spell_creature_blood_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_creature_blood_SpellScript);

		void FilterTargets(std::list<WorldObject*>& targets)
		{
			for (std::list<WorldObject*>::iterator itr = targets.begin(); itr != targets.end();)
			{
				if ((*itr)->GetTypeId() == TYPEID_UNIT && (*itr)->ToCreature() && !(*itr)->ToCreature()->IsCharmedOwnedByPlayerOrPlayer())
					++itr;
				else
					targets.erase(itr++);
			}
		...
		void Register() override
		{
			OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_creature_blood_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
		}
	};
