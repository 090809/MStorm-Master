				...
				for (Unit::ControlList::const_iterator itr = caster->m_Controlled.begin(); itr != caster->m_Controlled.end(); ++itr)
					if (Creature* Pet = (*itr)->ToCreature())
						if (Pet->IsAlive() &&
							...
							caster->ToPlayer()->GetSelectedUnit())
						{
							Pet->Attack(caster->ToPlayer()->GetSelectedUnit(), false);
							break;
						}
            }
...
class spell_mage_ice_shard : public SpellScriptLoader
{
public:
	spell_mage_ice_shard() : SpellScriptLoader("spell_mage_ice_shard") { }

	class spell_mage_ice_shard_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_mage_ice_shard_AuraScript);

	public:
		spell_mage_ice_shard_AuraScript()
		{
			caster = nullptr;
		}
	...
	private:
		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_MAGE_ICE_SPEAR))
				return false;
			return true;
		}
		...
		{
			PreventDefaultAction();
			caster->CastSpell(eventInfo.GetDamageInfo()->GetVictim(), 
				sSpellMgr->GetLastSpellInChain(SPELL_MAGE_ICE_SPEAR), true, NULL, NULL);
		}

		void Register() override
		{
			DoCheckProc += AuraCheckProcFn(spell_mage_ice_shard_AuraScript::CheckProc);
			OnEffectProc += AuraEffectProcFn(spell_mage_ice_shard_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
		}
	...
	AuraScript* GetAuraScript() const override
	{
		return new spell_mage_ice_shard_AuraScript();
	}
};
