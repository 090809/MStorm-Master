...
class spell_rogue_kiss_grip : public SpellScriptLoader
{
public:
	spell_rogue_kiss_grip() : SpellScriptLoader("spell_rogue_kiss_grip") { }

	class spell_rogue_kiss_grip_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_rogue_kiss_grip_SpellScript);

		void HandleDummy(SpellEffIndex /*effIndex*/)
		{
			int32 spellid = 49575; //49575
			Unit* caster = GetCaster();
			if (Unit* target = GetHitUnit())
			{
				if (caster->GetArmor() < target->GetArmor())
					...
					target->CastSpell(caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), spellid, true);
			}
		}

		void Register() override
		{
			OnEffectHitTarget += SpellEffectFn(spell_rogue_kiss_grip_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
		}
	...
	SpellScript* GetSpellScript() const override
	{
		return new spell_rogue_kiss_grip_SpellScript();
	}
};

class spell_rogue_cloth_of_shadow : public SpellScriptLoader
{
public:
	spell_rogue_cloth_of_shadow() : SpellScriptLoader("spell_rogue_cloth_of_shadow") { }

	class spell_rogue_cloth_of_shadow_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_rogue_cloth_of_shadow_AuraScript);

		void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
		{
			if (Player* player = GetTarget()->ToPlayer())
			{
				if (player->HasAura(600004) && urand(1, 100) < 41)
					player->CastSpell(player, 600026, true);
			}
		}

		void Register() override
		{
			OnEffectApply += AuraEffectApplyFn(spell_rogue_cloth_of_shadow_AuraScript::HandleApply, EFFECT_0, SPELL_AURA_MOD_ATTACKER_SPELL_HIT_CHANCE, AURA_EFFECT_HANDLE_REAL);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_rogue_cloth_of_shadow_AuraScript();
	}
};

void AddSC_rogue_spell_scripts()
{
	new spell_rogue_kiss_grip();
	...
	new spell_rogue_cloth_of_shadow();
}
