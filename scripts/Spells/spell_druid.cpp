...
//200830
class spell_dru_nature_regenate : public SpellScriptLoader
{
public:
	spell_dru_nature_regenate() : SpellScriptLoader("spell_dru_nature_regenate") { }

	class spell_dru_nature_regenate_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_dru_nature_regenate_AuraScript);

		bool Validate(SpellInfo const* /*spell*/) override
		{
			if (!sSpellMgr->GetSpellInfo(200830))
				return false;
			return true;
		}
		...
		void Absorb(AuraEffect* /*aurEff*/, DamageInfo & dmgInfo, uint32 & absorbAmount)
		{
			absorbAmount = CalculatePct(dmgInfo.GetDamage(), 100);
			GetCaster()->HealBySpell(GetCaster(), GetSpellInfo(), dmgInfo.GetDamage(), false);
		}

		void Register() override
		{
			DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dru_nature_regenate_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
			OnEffectAbsorb += AuraEffectAbsorbFn(spell_dru_nature_regenate_AuraScript::Absorb, EFFECT_0);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_dru_nature_regenate_AuraScript();
	}
};

class spell_dru_sov_rage : public SpellScriptLoader
{
public:
	spell_dru_sov_rage() : SpellScriptLoader("spell_dru_sov_rage") { }

	class spell_dru_sov_rage_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_dru_sov_rage_AuraScript);

		void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
		{
			// do not set speed if not in moonkin form
			if (GetUnitOwner()->GetShapeshiftForm() != FORM_MOONKIN)
				amount = 0;
		}

		void Register() override
		{
			DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dru_sov_rage_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_MOD_INCREASE_SPEED);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_dru_sov_rage_AuraScript();
	}
};
