...
public:
    ...
    ChatCommand* GetCommands() const override
    {
        static ChatCommand commandTable[] =
        {
			...
			{ "additemtome", rbac::RBAC_PERM_COMMAND_ADDITEMTOME, false, &HandleAddItemToMeCommand, "", NULL },
			{ "reward", rbac::RBAC_PERM_COMMAND_REWARD, false, &HandleRewardCommand, "", NULL },
			...
			{ "visualenchant",	  rbac::RBAC_PERM_COMMAND_VISUALENCHANT,	false, &HandleVisualEnchantCommand,	   "", NULL	},
			{ NULL,               0,                                  false, NULL,                           "", NULL }
        };
        return commandTable;
	}

	static bool HandleVisualEnchantCommand(ChatHandler* handler, char const* args) {
		if (!*args)
			return false;
		...
		uint32 param2 = uint32(atoul(strtok((char*)args, " ")));
		Item* item = target->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
		
		if (!item)
		{
			handler->SendSysMessage("Wrong item");
			return false;
		}

		SetEnchantVisual(target, item, param2);

		return true;
	}

	static bool HandleAddItemToMeCommand(ChatHandler* handler, char const* args)
	{
		if (!*args)
			return false;
	...
	static bool HandleRewardCommand(ChatHandler* handler, char const* args)
	{
		...
		uint32 itemId = sWorld->getIntConfig(COMMANDS_REWARD_ID);


		char const* ccount = strtok((char*)args, " ");
